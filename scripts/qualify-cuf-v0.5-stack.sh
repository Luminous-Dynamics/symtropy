...
#
# Local qualification gate for the stacked Causal Universe Fabric v0.1-v0.5.
# Run via: nix develop --command bash scripts/qualify-cuf-v0.5-stack.sh
...
CUF v0.2-v0.5 qualification requires the full/private Luminous Dynamics
...
printf '\n== Portable causal contracts + observation evidence ==\n'
cargo test -p symtropy-sim-contracts
cargo clippy -p symtropy-sim-contracts --all-targets -- -D warnings

printf '\n== World authority + fidelity + residency + evidence bundles ==\n'
...
printf '\nPASS: CUF v0.1-v0.5 local qualification gates\n'
