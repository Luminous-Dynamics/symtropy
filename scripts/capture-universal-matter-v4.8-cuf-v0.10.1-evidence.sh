...
#
# Capture evidence for the exact combined Universal Matter v4.8 + CUF v0.10.1
# staged tree. Output is deliberately outside the repository worktree.
...
if [[ "$staged_count" != "$EXPECTED_STAGED_PATHS" ]]; then
    printf 'ERROR: expected %s staged v4.8 paths before capture; found %s.\n' \
...
if [[ -z "$OUT" ]]; then
    OUT="/tmp/symtropy-um-v4.8-cuf-v0.10.1-${staged_tree:0:12}"
fi
...
printf '\nCombined qualification evidence capsule: %s\n' "$OUT"
printf 'Status: '
cat "$OUT/STATUS.txt"
printf 'Parent: %s\n' "$base_head"
