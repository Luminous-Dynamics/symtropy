\chapter{Chapter 13: The Thread That Unraveled Everything}\label{chapter-13-the-thread-that-unraveled-everything}

\emph{In which we ask the question we should have asked first, and discover that everything we believed was wrong.}
...
We had never tested this. In every experiment from Chapter 5 through Chapter 12, agents followed the same FEP-style behavioral gradient --- a handcrafted function that explicitly directs agents toward resonant partners. We had varied the environment in dozens of ways --- scarcity, adversaries, topology, volatility, dimensionality --- but we had never varied the \emph{controller}. We had tested our thesis against every environmental challenge we could imagine while leaving the one variable that mattered completely untouched.
...
\tightlist
\item
  \textbf{FEP\_GRADIENT}: our original controller --- seek partners, seek wells, flee danger, explore
\item
  \textbf{WELL\_ONLY}: navigate to the nearest energy well, ignore other agents entirely
\item
  \textbf{GREEDY}: look in eight directions, move toward maximum energy gain
\item
  \textbf{PARTNER\_ONLY}: navigate to the nearest resonant partner, ignore wells
\item
  \textbf{RANDOM}: uniform random direction each tick
\item
  \textbf{STATIONARY}: don't move at all
\end{enumerate}
...
WELL\_ONLY agents --- who never seek social contact, who navigate exclusively toward energy wells --- cooperated 1.22 million times across 8,000 ticks. More than the FEP gradient's 1.08 million. They survived at 20 out of 20. The FEP gradient survived at 10.8.
...
The cooperation we had spent twelve chapters analyzing, testing, and celebrating was not caused by our carefully designed behavioral gradient. It was caused by \emph{being near energy wells at the same time as other agents}. Any controller that brings agents near wells --- or that simply doesn't move them away --- produces cooperation as a side effect of spatial proximity.

\section{The FEP Gradient Was the Problem}\label{the-fep-gradient-was-the-problem}

The most disturbing column in the data was survival. Our FEP gradient --- the sophisticated, multi-component, carefully weighted behavioral rule that we had built the entire book around --- produced the \emph{worst survival of any controller}.
...
\section{What By-Product Mutualism Means}\label{what-by-product-mutualism-means}

In behavioral ecology, \emph{by-product mutualism} describes a situation where one organism's selfish actions incidentally benefit others nearby. A vulture circling a carcass attracts other vultures --- not through communication or cooperation, but because the circling is visible and other vultures are watching. The first vulture doesn't intend to share; it intends to eat. The social structure (a group of vultures at a carcass) is a by-product of individual resource-seeking.
...
\emph{Next: Chapter 14 --- Seven Failed Rescues}
