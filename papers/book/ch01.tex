\chapter{Chapter 1: Why Physics, Not Utility Functions}\label{chapter-1-why-physics-not-utility-functions}

\emph{In which we ask whether cooperation needs a reason, and discover that thermodynamics is sufficient.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{The Standard Story}\label{the-standard-story}
...
This is the story told by Axelrod's \emph{Evolution of Cooperation} (1984), by Nowak's \emph{SuperCooperators} (2011), by the vast literature on evolutionary game theory from Maynard Smith (1982) through Sigmund (2010). It is a story about strategy, payoff, and optimization.

It is not our story.

\section{What We Did Instead}\label{what-we-did-instead}
...
This is not a metaphor. The energy is tracked in joules. The entropy increases monotonically. The Helmholtz free energy is computed as F = U - TS. The Landauer bound (2.87 $\times$ 10$^{-}$²¹ joules per bit at body temperature) establishes the minimum energy cost of information processing. Every action costs energy. Every collision dissipates heat. Every tick, the second law takes its tax.
...
The question this book asks is simple: \textbf{Is thermodynamic enforcement sufficient to produce cooperation?}

Not cooperation as a strategy. Not cooperation as an evolutionary stable state. Cooperation as a \emph{physical inevitability} --- as certain as heat flowing from hot to cold, as predictable as entropy increasing in a closed system.
...
\section{A Note on Honesty}\label{a-note-on-honesty}

This book makes no claim about consciousness. The engine uses an ``integration metric'' ($\Phi$) inspired by Tononi's Integrated Information Theory, but we use it as a coupling parameter, not as a claim about subjective experience. Where we write ``consciousness'' in code and equations, we mean ``a scalar value that modulates physics.'' Nothing more.
...
\emph{Next: Chapter 2 --- Five Channels: How Integration Metrics Couple to Physics}
