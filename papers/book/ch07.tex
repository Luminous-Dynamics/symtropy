\chapter{Chapter 7: Social Bonds and the Physics of Friendship}\label{chapter-7-social-bonds-and-the-physics-of-friendship}

\emph{In which physical constraints between resonant agents improve survival by 70\%, and we discover that friendship is a spring.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{The Constraint Hypothesis}\label{the-constraint-hypothesis}

Chapters 5 and 6 established that agents cluster and cooperate through the FEP gradient. But the gradient only \emph{attracts} --- it creates no lasting connection. Once an agent's energy rises above 50\%, the well-seeking urgency drops, and the social component weakens. Agents drift apart.

Real social bonds don't work this way. Friendships persist through periods of comfort. Family ties endure when no one needs help. The question is: does adding \emph{persistence} to social connections --- physical constraints that hold resonant agents together --- change cooperation outcomes?
...
\section{Finding 19: Bonds Transform Survival}\label{finding-19-bonds-transform-survival}

Twenty agents, six distinct harmony archetypes (ensuring selective bonding, not universal), ten seeds each. BONDS condition forms springs between high-resonance pairs. NO\_BONDS condition is the standard FEP-only baseline.
...
\bottomrule\noalign{}
\endlastfoot
Survival & 11.4/20 (57\%) & 19.4/20 (97\%) & \textbf{p=0.001, d=2.44} \\
...
\section{The Dunbar Limit}\label{the-dunbar-limit}
...
\endlastfoot
4 & 70\% & 2.5 & 1.1 \\
\textbf{12} & \textbf{90\%} & \textbf{3.3} & \textbf{3.0} \\
20 & 70\% & 4.0 & 4.0 \\
48 & 50\% & 3.2 & 12.8 \\
96 & 60\% & 4.2 & 14.7 \\
\end{longtable}
...
At N=1,000 (Chapter 4), cluster size reached 72.4 with 14 clusters --- still stable. The ``Dunbar number'' of this engine is not a number but a \emph{density}: how many agents per unit area can sustain resonance within harmony range. This scales with the arena, not with the population.
...
Result: 18.2 out of 20 agents migrated, traveling a mean distance of 3,274 units --- more than thirty times the arena width. All twenty survived. The FEP gradient, sensing declining well returns, redirected agents toward the remaining wells without any coordination or communication.
...
\section{Generational Wealth}\label{generational-wealth}
...
The policy implication --- again offered cautiously --- is that \emph{mobility} (the ability to form new cooperative relationships) matters more than \emph{inheritance} (starting position). An engine with both mobility and cooperation produces equality. An engine with neither produces the Matthew Effect.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{Next: Chapter 8 --- Raiders, Evolvers, and the Solidarity Response}
