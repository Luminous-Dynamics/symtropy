...
import ast, contextlib, hashlib, importlib, io, json, subprocess, sys, tempfile, unittest
...
class ParserTests(unittest.TestCase):
    ...
    def test_contract_cli_emits_exact_subject_identity(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); path=root/'contracts'/'pilot.json'; path.parent.mkdir()
            raw=(json.dumps(base_contract(),sort_keys=True,separators=(',',':'))+'\n').encode()
            path.write_bytes(raw); digest=hashlib.sha256(raw).hexdigest(); out=io.StringIO()
            with contextlib.redirect_stdout(out):
                rc=contract.main(['--contract-root',str(root),'--contract-path','contracts/pilot.json','--expected-sha256',digest])
            self.assertEqual(rc,0); text=out.getvalue(); self.assertIn('QUALIFICATION_CONTRACT_V1 PASS',text); self.assertIn('subject_head_sha='+ONE40,text)
...
class EngineTests(unittest.TestCase):
    ...
    def test_workflow_python_execution_is_bytecode_free(self):
        repo_root=ROOT.parents[1]
        text=(repo_root/'.github/workflows/qual-001b-authoritative-verifier-v1.yml').read_text()
        self.assertNotIn('python -m py_compile',text)
        self.assertIn("python -B - <<'PY'",text)
        self.assertIn('python -B "$VERIFIER_DIR/tools/qualification/qualification_contract_v1.py"',text)
        self.assertIn('python -B "$VERIFIER_DIR/tools/qualification/authoritative_verifier_v1.py"',text)
