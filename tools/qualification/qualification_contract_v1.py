...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        rel = repo_path(args.contract_path, "contract_path")
        path = resolve_regular_file(args.contract_root, rel, "contract_path")
        value, digest = load_contract(path)
        validate_contract(value)
        if digest != args.expected_sha256:
            _fail("contract SHA-256 does not match expected digest")
    except ContractValidationError as exc:
        print(f"QUALIFICATION_CONTRACT_V1 FAIL: {exc}", file=sys.stderr)
        return 2
    print("QUALIFICATION_CONTRACT_V1 PASS")
    print(f"contract_id={value['contract_id']}")
    print(f"contract_sha256={digest}")
    print(f"subject_repository={value['subject_repository']}")
    print(f"subject_head_sha={value['subject_head_sha']}")
    print(f"subject_tree_sha={value['subject_tree_sha']}")
    print(f"manifest_path={value['manifest_path']}")
    print(f"suite_profile_path={value['suite_profile_path']}")
    return 0
