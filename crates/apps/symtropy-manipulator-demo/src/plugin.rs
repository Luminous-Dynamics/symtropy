...
fn step_phi_arm(
    ...
    // Channels 3/4 must be LIVE measurements: the agent's
    // integration_signal() computes per-channel differentiation over
    // observation[2..], so constants zero the Φ input every tick (the
    // placeholder bug fixed 2026-07-10). Control effort = saturating norm
    // of the measured external forces; stiffness = last tick's admittance
    // stiffness (itself Φ-driven, closing the loop the demo advertises).
    let force_norm = (external_forces[0] * external_forces[0]
        + external_forces[1] * external_forces[1]
        + external_forces[2] * external_forces[2])
        .sqrt();
    let control_effort = force_norm / (1.0 + force_norm);
    let prev_stiffness = phi_arm.stiffness;
