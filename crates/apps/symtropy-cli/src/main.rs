...
/// Known robotics demos. Each is a workspace-member crate under
/// `symtropy/crates/apps/symtropy-<name>-demo/` (since the 2026-06
/// apps/ reorg + 2026-07 restoration they are ordinary members of the
/// symtropy workspace, on the workspace Bevy version). The `run`
/// subcommand shells out to `cargo run --release` inside each demo's
/// crate directory; bin names equal the package names.
struct DemoEntry {
    name: &'static str,
    bin: &'static str,
...
fn cmd_calibrate(mut args: impl Iterator<Item = String>) -> Result<(), String> {
    ...
    let output = Command::new("cargo")
        ...
        .arg("--release")
        .arg("--quiet")
        // robotics-bridge is workspace-EXCLUDED (AGPL split), so `-p`
        // from the workspace root cannot resolve it — address it by
        // manifest path instead.
        .arg("--manifest-path")
        .arg("crates/bridges/symtropy-robotics-bridge/Cargo.toml")
        .arg("--example")
        .arg("phi_trace")
        .output()
    ...
    println!();
    println!(" To apply: edit `crates/apps/symtropy-{platform}-demo/src/plugin.rs` and set:");
...
fn locate_repo_root() -> Result<PathBuf, String> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        if ancestor.ends_with("symtropy")
            ...
            && ancestor
                .join("crates/bridges/symtropy-robotics-bridge")
                .exists()
        {
        ...
        if ws.join("Cargo.toml").exists()
            && ws.join("crates/bridges/symtropy-robotics-bridge").exists()
        {
            return Ok(ws);
        }
    }
...
fn locate_demo_crate(name: &str) -> Result<PathBuf, String> {
    let rel = format!("symtropy/crates/apps/symtropy-{name}-demo/Cargo.toml");
...
mod tests {
    ...
    fn demo_bin_names_match_convention() {
        // Bin name == package name == `symtropy-<short>-demo` (no demo
        // declares a [[bin]] section, so cargo uses the package name).
        for d in DEMOS {
            let expected = format!("symtropy-{}-demo", d.name);
            assert_eq!(d.bin, expected, "demo `{}` bin mismatch", d.name);
        }
    }

    #[test]
    fn demo_crates_exist_at_expected_paths() {
        // Guards against the 2026-06 reorg failure mode: the CLI's path
        // scheme silently drifting from where the demo crates live.
        let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let apps_dir = manifest_dir.parent().expect("apps dir");
        for d in DEMOS {
            let crate_dir = apps_dir.join(format!("symtropy-{}-demo", d.name));
            assert!(
                crate_dir.join("Cargo.toml").exists(),
                "demo `{}` not found at {} — path scheme drifted?",
                d.name,
                crate_dir.display()
            );
        }
