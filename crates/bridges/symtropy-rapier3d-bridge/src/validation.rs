...
//! Rapier 0.18 adapter for Symtropy's engine-neutral validation contract.
//!
//! This module translates the deliberately narrow v0.1 scenario vocabulary
//! into Rapier state and converts the bridge's generation-bearing observations
//! back into stable validation-body identities. Rapier remains independent
//! comparison evidence, never canonical Symtropy physical truth.
...
use symtropy_physics::validation::{
    ...
    ValidationBodyId, PHYSICS_TRACE_SCHEMA_VERSION,
};
use symtropy_physics::world::NoOpCallback;

use crate::RapierPhysicsBridge;

/// Exact external-engine generation implemented by this adapter.
pub const RAPIER_REFERENCE_VERSION: &str = "0.18";

/// Rapier-specific failure while adapting a validated common scenario.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum RapierValidationError {
...
/// Execute the common v0.1 scenario using the pinned Rapier reference backend.
pub fn run_rapier_scenario_3d(
...
fn insert_body(
    ...
    if spec.body_type == ScenarioBodyType::Dynamic {
        // Put the declared mass on the collider so Rapier derives the matching
        // shape inertia from exactly that total mass. Do not add the mass on top
        // of Rapier's default-density collider mass.
...
mod tests {
    use super::*;
    use symtropy_physics::validation::{
        compare_traces_3d, run_native_scenario_3d, ScenarioBody3d, ScenarioBodyType,
        ScenarioShape3d, ValidationBodyId, PHYSICS_SCENARIO_SCHEMA_VERSION,
    };
    ...
    fn native_and_rapier_share_identity_schedule_and_finite_metrics() {
        ...
        // This is a broad sanity envelope, not a physics qualification claim.
