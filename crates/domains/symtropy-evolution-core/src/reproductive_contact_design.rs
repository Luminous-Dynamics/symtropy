...
const DESIGN_DOMAIN: &[u8] = b"symtropy:evolution:reproductive-contact-study-design:v1\0";

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize)]
pub struct ReproductiveContactStudyId(String);

impl ReproductiveContactStudyId {
    pub fn new(value: impl Into<String>) -> Result<Self, EvolutionError> {
        let value = value.into();
        validate_text("ReproductiveContactStudyId", &value)?;
        Ok(Self(value))
    }
    pub fn as_str(&self) -> &str { &self.0 }
}
impl<'de> Deserialize<'de> for ReproductiveContactStudyId {
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize)]
pub struct ReproductiveOpportunityId(String);

impl ReproductiveOpportunityId {
    pub fn new(value: impl Into<String>) -> Result<Self, EvolutionError> {
        let value = value.into();
        validate_text("ReproductiveOpportunityId", &value)?;
        Ok(Self(value))
    }
    pub fn as_str(&self) -> &str { &self.0 }
}
impl<'de> Deserialize<'de> for ReproductiveOpportunityId {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where D: Deserializer<'de> {
        let value = String::deserialize(deserializer)?;
        Self::new(value).map_err(<D::Error as serde::de::Error>::custom)
...
impl ReproductiveOpportunityDeclaration {
    ...
    fn update_digest(&self, digest: &mut Sha256) {
        ...
        update_authority_digest(digest, &self.contact_zone_evidence);
    }
}
...
impl ReproductiveContactStudyDesign {
    ...
    pub fn declare(
        ...
        if lineage_a == lineage_b {
            return Err(ReproductiveContactDesignError::SameLineageIdentity);
        }
        if end_generation.0 < start_generation.0 {
            return Err(ReproductiveContactDesignError::InvalidGenerationInterval);
        }
        let mut by_id = BTreeMap::new();
        for opportunity in opportunities {
            opportunity.validate_local()?;
            if opportunity.generation.0 < start_generation.0
                || opportunity.generation.0 > end_generation.0
            {
                return Err(ReproductiveContactDesignError::OpportunityOutsideInterval(
                    opportunity.opportunity_id,
                ));
            }
            if let ReproductiveContactContextPolicy::ExactContext { context_digest } =
                &context_policy
            {
                if opportunity.context_digest != *context_digest {
                    return Err(ReproductiveContactDesignError::UndeclaredContextDrift(
                        opportunity.opportunity_id,
                    ));
                }
            }
        ...
        if by_id.is_empty() {
            return Err(ReproductiveContactDesignError::EmptyOpportunityCensus);
        }
        let design = Self {
            ...
            end_generation,
            context_policy,
            opportunities: by_id.into_values().collect(),
            opportunity_definition_authority,
            contact_protocol,
            pairing_protocol,
            mating_protocol,
            conception_protocol,
            offspring_viability_protocol,
            offspring_fertility_protocol,
            parentage_authority,
            gene_flow_materialization_authority,
    ...
    pub fn canonical_digest(&self) -> Result<ReproductiveContactStudyDesignDigest, ReproductiveContactDesignError> {
        ...
        update_authority_digest(&mut digest, &self.lineage_a);
        update_authority_digest(&mut digest, &self.lineage_b);
        ...
        for opportunity in &self.opportunities { opportunity.update_digest(&mut digest); }
        for authority in [
            &self.opportunity_definition_authority,
            &self.contact_protocol,
            &self.pairing_protocol,
            &self.mating_protocol,
            &self.conception_protocol,
            &self.offspring_viability_protocol,
            &self.offspring_fertility_protocol,
            &self.parentage_authority,
            &self.gene_flow_materialization_authority,
            &self.demography_accounting_authority,
            &self.missing_data_authority,
        ] { update_authority_digest(&mut digest, authority); }
        Ok(ReproductiveContactStudyDesignDigest(digest.finalize().into()))
    }
    ...
    fn validate_local(&self) -> Result<(), ReproductiveContactDesignError> {
        ...
        for opportunity in &self.opportunities { opportunity.validate_local()?; }
...
impl fmt::Debug for ReproductiveContactStudyDesignDigest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "ReproductiveContactStudyDesignDigest(")?; fmt_hex(&self.0, f)?; write!(f, ")")
    }
}
...
impl<'a> ValidatedReproductiveContactStudyDesign<'a> {
    ...
    pub fn validate_current(
        ...
        let recomputed = ReproductiveContactStudyDesign::declare(
            design.study_id.clone(), lineage_a, lineage_b,
            design.start_generation, design.end_generation, context_policy, opportunities,
            opportunity_definition_authority, contact_protocol, pairing_protocol, mating_protocol,
            conception_protocol, offspring_viability_protocol, offspring_fertility_protocol,
            parentage_authority, gene_flow_materialization_authority,
...
pub enum ReproductiveContactDesignError {
    UnsupportedVersion(u32), SameLineageIdentity, SameIndividualPair,
    InvalidGenerationInterval, EmptyOpportunityCensus,
    DuplicateOpportunity(ReproductiveOpportunityId),
    OpportunityOutsideInterval(ReproductiveOpportunityId),
    UndeclaredContextDrift(ReproductiveOpportunityId),
