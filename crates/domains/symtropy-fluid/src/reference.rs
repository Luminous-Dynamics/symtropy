...
impl PeriodicMac2d {
    ...
    pub fn project_velocity(&mut self, dt_s: f64) -> Result<ProjectionReport, ReferenceStepError> {
        ...
        let before = self.divergence_field();
        ...
        let mut next_pressure = vec![0.0; self.pressure_pa.len()];
        ...
        for _ in 0..self.config.pressure_iterations {
            for j in 0..self.config.ny {
                ...
                for i in 0..self.config.nx {
                    ...
                    next_pressure[index] = ((self.pressure_pa[self.index(east, j)]
                        + self.pressure_pa[self.index(west, j)])
                        * inv_dx2
                        + (self.pressure_pa[self.index(i, north)]
            ...
            std::mem::swap(&mut self.pressure_pa, &mut next_pressure);
        }

        let pressure_mean = mean(&self.pressure_pa);
        for pressure in &mut self.pressure_pa {
            *pressure -= pressure_mean;
        }

        let pressure_residual = self.pressure_residual_rms(&rhs);
        ...
        for j in 0..self.config.ny {
            ...
            for i in 0..self.config.nx {
                let west = previous(i, self.config.nx);
                let index = self.index(i, j);
                self.u_faces[index] -= dt_over_rho
                    * (self.pressure_pa[index] - self.pressure_pa[self.index(west, j)])
                    / dx;
                self.v_faces[index] -= dt_over_rho
                    * (self.pressure_pa[index] - self.pressure_pa[self.index(i, south)])
                    / dy;
            }
        }

        let after_rms = self.divergence_rms_per_s();
        Ok(ProjectionReport {
        ...
        F: FnMut([f64; 2], f64) -> [f64; 2],
    {
        ...
        let dx = self.config.dx();
        let dy = self.config.dy();
        let max_u = max_abs(&self.u_faces);
        let max_v = max_abs(&self.v_faces);
        let max_advective_cfl = dt_s * (max_u / dx + max_v / dy);
        if max_advective_cfl > self.config.max_advective_cfl {
            return Err(ReferenceStepError::CflLimitExceeded {
                observed: max_advective_cfl,
        ...
        self.u_faces = next_u;
        self.v_faces = next_v;
        let projection = self.project_velocity(dt_s)?;
    ...
    fn divergence_field(&self) -> Vec<f64> {
        let dx = self.config.dx();
        let dy = self.config.dy();
        let mut divergence = vec![0.0; self.u_faces.len()];
        for j in 0..self.config.ny {
            let north = next(j, self.config.ny);
            for i in 0..self.config.nx {
                let east = next(i, self.config.nx);
    ...
    fn pressure_residual_rms(&self, rhs: &[f64]) -> f64 {
        let inv_dx2 = 1.0 / self.config.dx().powi(2);
        let inv_dy2 = 1.0 / self.config.dy().powi(2);
        ...
        for j in 0..self.config.ny {
            ...
            for i in 0..self.config.nx {
                ...
                let east = next(i, self.config.nx);
                let index = self.index(i, j);
                let laplacian = (self.pressure_pa[self.index(east, j)]
                    - 2.0 * self.pressure_pa[index]
                    + self.pressure_pa[self.index(west, j)])
                    * inv_dx2
                    + (self.pressure_pa[self.index(i, north)]
                        - 2.0 * self.pressure_pa[index]
        ...
        (residual_squared / self.pressure_pa.len() as f64).sqrt()
    }
