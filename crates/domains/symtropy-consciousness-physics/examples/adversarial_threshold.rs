...
fn main() {
    ...
    println!(
        "fraction,seed,coop_alive,adv_alive,clustering,polarization,coop_energy,adv_energy,cooperation"
    );
    ...
    for &frac in &fractions {
        ...
        eprintln!(
            "  {:.0}% adversaries: coop_alive={:.1}/{:.0}, adv_alive={:.1}/{:.0}, cluster={:.2}, coop_events={:.0}",
            frac * 100.0,
            results.iter().map(|r| r.cooperator_alive).sum::<f64>() / n,
            nc,
            results.iter().map(|r| r.adversary_alive).sum::<f64>() / n,
            na,
