...
//! Functional engineering constraints over externally supplied evidence.
//!
//! This layer does not become a physics oracle and does not authorize operation.
//! It asks whether a concrete role binding is supported by supplied engineering
//! facts and preserves the distinction between known failure and missing or
//! ambiguous evidence.
    ...
    fn evaluate_constraint(
            ...
            .all(|result| result.outcome == ConstraintOutcome::Unsatisfied)
        {
            return ConstraintEvaluation {
                constraint_id: constraint.id.clone(),
                outcome: ConstraintOutcome::Unsatisfied,
                reason: individual[0].reason.clone(),
                supporting_facts,
            };
        }
...
mod tests {
    ...
    #[test]
    fn uncertain_measurement_straddling_limit_remains_unknown() {
