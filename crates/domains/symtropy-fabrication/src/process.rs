...
fn validate_resulting_matter(bindings: &[MatterBinding]) -> Result<(), ProcessError> {
    ...
    for (index, binding) in bindings.iter().enumerate() {
        if bindings[..index]
            .iter()
            .any(|existing| existing.allocation_id == binding.allocation_id)
        {
            return Err(ProcessError::DuplicateResultingMatterAllocation(
                binding.allocation_id.clone(),
            ));
        }
    }
    Ok(())
...
pub enum ProcessError {
    ...
    DuplicateResultingMatterAllocation(StableId),
...
impl fmt::Display for ProcessError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::DuplicateResultingMatterAllocation(allocation) => write!(
                formatter,
                "resulting matter allocation {allocation} is listed more than once"
            ),
...
mod tests {
    ...
    #[test]
    fn duplicate_resulting_allocation_is_rejected() {
        let workpiece = workpiece("workpiece:plate", 7);
        let mut execution = ProcessExecution::begin(
            ProcessExecutionId::new(id("process-execution:weld")),
            &weld_spec(),
        ...
        assert!(matches!(
            result,
            Err(ProcessError::DuplicateResultingMatterAllocation(_))
        ));
    }
