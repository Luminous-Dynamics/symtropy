...
impl IncrementalDecompositionState {
    ...
    pub fn advance(
        ...
        let (snapshot, changed_cells, recompute_cells) =
            incremental_snapshot(self, profile, domain, &successor_census)?;
        let state = Self {
        ...
        Ok(IncrementalRecomputeOutcome::Updated(Box::new(
...
fn all_face_neighbors(cell: CellCoord, domain: &AnalysisDomain) -> Vec<CellCoord> {
    ...
    let candidates = [
        cell.x
            .checked_sub(1)
            .map(|x| CellCoord::new(x, cell.y, cell.z)),
        cell.x
            .checked_add(1)
            .map(|x| CellCoord::new(x, cell.y, cell.z)),
        cell.y
            .checked_sub(1)
            .map(|y| CellCoord::new(cell.x, y, cell.z)),
        cell.y
            .checked_add(1)
            .map(|y| CellCoord::new(cell.x, y, cell.z)),
        cell.z
            .checked_sub(1)
            .map(|z| CellCoord::new(cell.x, cell.y, z)),
        cell.z
            .checked_add(1)
            .map(|z| CellCoord::new(cell.x, cell.y, z)),
    ];
