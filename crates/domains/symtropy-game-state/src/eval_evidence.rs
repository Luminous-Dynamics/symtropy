...
impl EvidenceChainV1 {
    ...
    pub fn verify(&self) -> Result<(), EvidenceError> {
        ...
        for (index, event) in self.events.iter().enumerate() {
            ...
            match &event.kind {
                ...
                EvidenceEventKindV1::BlindOutputCommitted { trial, stage } => {
                    if revealed {
                        return Err(EvidenceError::InvalidEventOrder("blind output after reveal"));
                    }
                    let (_, requirements) =
    ...
    fn blinding_policy(
        &self,
    ) -> Option<(&BlindAssignmentCommitmentV1, &RevealRequirementsV1)> {
    ...
    fn contract_role_is_committed(&self, role: &str) -> bool {
        self.events.iter().any(|event| {
            matches!(&event.kind, EvidenceEventKindV1::ContractCommitted)
                && event.artifacts.iter().any(|artifact| artifact.role() == role)
        })
    }
...
fn validate_stage_profile(requirements: &RevealRequirementsV1) -> Result<(), EvidenceError> {
    ...
    if requirements.stages().windows(2).any(|pair| pair[0] >= pair[1]) {
        return Err(EvidenceError::InvalidBlindStageProfile(
...
fn validate_canonical_collections(
    ...
    if let Some(pair) = artifacts.windows(2).find(|pair| pair[0].role == pair[1].role) {
...
fn verify_canonical_collections(event: &EvidenceEventV1) -> Result<(), EvidenceError> {
    ...
    if event.bindings.windows(2).any(|pair| pair[0].key >= pair[1].key) {
...
impl fmt::Display for EvidenceError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ParentDigestMismatch => formatter
                .write_str("evidence parent digest does not match the preceding event"),
...
mod tests {
    ...
    fn committed_chain_with(
        stages: Vec<BlindStageV1>,
    ) -> (EvidenceChainV1, BlindRevealV1) {
    ...
    fn scenario_commitment_must_reference_scenario_contract_role() {
        ...
        chain.append_contract(vec![wrong], Vec::new()).expect("contract");
        let commitment = BlindAssignmentCommitmentV1::commit(
            "scenario-role",
            digest,
    ...
    fn score_requires_single_authoritative_oracle_and_is_terminal() {
        ...
        chain
            ...
            .expect("oracle");
        assert_eq!(
            chain.append_post_reveal_oracle(
                vec![artifact("oracle-2", b"other")],
                Vec::new(),
            ),
