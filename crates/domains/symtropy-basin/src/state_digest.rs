...
//! Canonical identity for complete `BasinWorld` causal state.
//!
//! This module hashes stored authoritative state, not metrics or serialized Rust
//! memory. Any state capable of influencing a future basin step belongs here.
...
impl BasinWorld {
    /// Returns the canonical digest of the complete stored Basin authority state.
    ///
    /// The encoding deliberately avoids Rust memory layout, serde output,
    /// platform-sized integer representation, and enum discriminants.
    pub fn causal_state_digest(&self) -> Result<TypedDigest32, BasinStateDigestError> {
        ...
        if self.cells.len() != expected_cells {
            return Err(BasinStateDigestError::CellCountMismatch {
        ...
        hash_u64(&mut hasher, canonical_len("cells", self.cells.len())?);
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CellCountMismatch { expected, actual } => write!(
                formatter,
                "basin cell count mismatch: expected {expected}, found {actual}"
            ),
...
mod tests {
    use super::*;
    use crate::{BasinIntervention, SystemId};
    ...
    #[test]
    fn hidden_spatial_cell_change_changes_identity() {
        let a = BasinWorld::old_waterworks(8, 5);
        let mut b = a.clone();
        b.cells[0].soil.carbon += 0.001;
    ...
    #[test]
    fn signal_and_civic_order_are_part_of_stored_identity() {
        ...
        let mut c = BasinWorld::old_waterworks(8, 5);
        c.civic_claims[0].opposition.push(crate::FactionId(99));
        assert_ne!(digest(&a), digest(&c));
    }

    #[test]
    fn intervention_and_step_change_causal_state_identity() {
        let mut world = BasinWorld::old_waterworks(8, 5);
        let initial = digest(&world);
        world.apply(BasinIntervention::WillowPlanting);
        let after_intervention = digest(&world);
        assert_ne!(initial, after_intervention);
        world.step();
        assert_ne!(after_intervention, digest(&world));
    }
