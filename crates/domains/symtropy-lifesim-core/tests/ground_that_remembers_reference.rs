...
fn arithmetic_failures_are_atomic() {
    let mut owner = owner();
    owner.patches.get_mut(&USED_PATCH).unwrap().disturbance_units = u32::MAX;
    let observation = owner.observe(USED_PATCH).unwrap();
...
fn downstream_interaction_changes_only_after_world_state_changes() {
    ...
    assert_eq!(light_before.support_cost_units, heavy_before.support_cost_units);
