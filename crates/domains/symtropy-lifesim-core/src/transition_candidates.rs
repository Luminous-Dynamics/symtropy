...
//!
//! `information_transition::InformationTransitionRegistry::plan` intentionally
//! freezes one structural shortest-path behavior. That is useful as a primitive,
//! but raw edge count must not become canonical ecological fidelity policy.
//! This module exposes the independent discovery step: enumerate every simple
...
/// Explicit search envelope for candidate discovery.
...
/// One structurally valid path and the authority/evidence consequences implied by
/// its registered transition definitions.
///
/// This is a candidate, not a canonical selection. Higher orchestration still
/// resolves applicability domains, external authority, transform qualification,
/// closure acceptance, spatiotemporal validity, and canonical selection policy.
...
impl TransitionCandidatePath {
    ...
    /// still require #306/#307 applicability proof or later policy validation.
...
/// Complete candidate set inside a search envelope that was proven not to
/// truncate any additional simple path under that envelope.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl InformationTransitionRegistry {
    ...
    /// only to make output/replay stable. That ordering is explicitly **not** a
    /// semantic preference. Canonical fidelity selection belongs to #272/#308.
...
impl fmt::Display for TransitionCandidateError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ZeroMaxEdges => write!(formatter, "transition candidate search max_edges must be > 0"),
...
mod tests {
    ...
    fn graph(policy: &InformationPolicyRegistry) -> InformationTransitionRegistry {
        ...
        // Register deliberately out of traversal order. BTreeMap transition-key
        // ordering, not bootstrap order, controls deterministic discovery.
        for transition in [
            edge(CD, C, D),
            edge(BD, B, D),
            edge(AC, A, C),
