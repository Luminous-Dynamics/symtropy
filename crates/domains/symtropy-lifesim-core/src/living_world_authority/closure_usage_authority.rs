...
//! This layer measures qualified use from an explicit validation anchor and
//! refuses to extend beyond the registered horizon unless a new anchor is
...
//! theorem. It tracks evidence age in canonical ticks. `ClosureValidationAnchor`
//! intentionally has no public constructor: this module consumes anchors; a
//! later exact/measurement/shadow-validation authority must mint them.
...
impl ClosureUsagePolicyDefinition {
    pub const fn new(
        ...
        Self { key, process, information, lineage, representation, maximum_use }
    }
    pub const fn key(self) -> ClosureUsagePolicyKey { self.key }
    pub const fn process(self) -> ProcessKey { self.process }
    pub const fn information(self) -> EcologicalInformation { self.information }
    pub const fn lineage(self) -> EvidenceLineageToken { self.lineage }
    pub const fn representation(self) -> RepresentationKey { self.representation }
    pub const fn maximum_use(self) -> MaximumClosureUseTicks { self.maximum_use }
}
...
impl ClosureUsagePolicyAuthorityStamp {
    pub const fn key(&self) -> ClosureUsagePolicyRegistryKey { self.key }
    ...
    pub fn definitions(&self) -> &BTreeMap<ClosureUsagePolicyKey, ClosureUsagePolicyDefinition> {
        &self.definitions
    }
}
...
impl ClosureUsagePolicyRegistryBuilder {
    pub const fn new(key: ClosureUsagePolicyRegistryKey) -> Self {
        Self { key, definitions: BTreeMap::new() }
    }
    ...
    pub fn register(
        ...
        if let Some(existing) = self.definitions.get(&key) {
            if existing == &definition { return Ok(()); }
            return Err(ClosureUsageAuthorityError::ConflictingUsagePolicyRegistration { policy: key });
        }
    ...
    pub fn seal(
        ...
        information_policy.authority_stamp().validate_registry(information_policy.registry())
            .map_err(ClosureUsageAuthorityError::InformationPolicyIdentity)?;
        closures.validate_current(information_policy)
            .map_err(ClosureUsageAuthorityError::ClosureQualification)?;
        acceptances.validate_current(information_policy)
            .map_err(ClosureUsageAuthorityError::ClosureAcceptance)?;
        spatiotemporal.validate_current(information_policy)
        ...
        Ok(ClosureUsagePolicyRegistry { authority, definitions: self.definitions })
    }
}
...
impl ClosureUsagePolicyRegistry {
    pub const fn authority_stamp(&self) -> &ClosureUsagePolicyAuthorityStamp { &self.authority }
    pub fn definition(&self, key: ClosureUsagePolicyKey) -> Option<ClosureUsagePolicyDefinition> {
...
fn validate_definition(
    ...
    let process = information_policy.registry().resolve_process(definition.process())
    ...
    let permits_closure = process.profile().requirements().iter().any(|requirement| {
        requirement.information() == definition.information()
            && matches!(requirement.evidence(), EvidenceRequirement::ExactOrQualifiedClosure(_))
    });
    if !permits_closure {
        return Err(ClosureUsageAuthorityError::ProcessDoesNotPermitClosure {
            process: definition.process(), information: definition.information(),
        });
    }

    let qualification = closures.resolve(information_policy, definition.lineage())
        .map_err(ClosureUsageAuthorityError::ClosureQualification)?;
    let qualified_horizon = qualification.qualification().horizon().get();
    if definition.maximum_use().get() > qualified_horizon {
        return Err(ClosureUsageAuthorityError::UsageHorizonExceedsQualification {
            policy: definition.key(), maximum_use: definition.maximum_use(), qualified_horizon,
        });
    }

    let representation = information_policy.registry().resolve_representation(definition.representation())
        .map_err(ClosureUsageAuthorityError::InformationRegistry)?;
    let exact_evidence = qualification.qualification().evidence();
    let carries = representation.capabilities().claims().iter().any(|(available, evidence)| {
...
/// Opaque proof receipt. Deliberately no public constructor.
#[derive(Debug, Clone, PartialEq, Eq)]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ClosureUsageAuthorityScope { id: u128, version: u32 }
...
impl ClosureUsageReceiptId {
    pub const fn scope(self) -> ClosureUsageAuthorityScope { self.scope }
    pub const fn revision(self) -> ClosureUsageRevision { self.revision }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ClosureUsageWindow { start_exclusive: CanonicalTick, end_inclusive: CanonicalTick }
impl ClosureUsageWindow {
        ...
        -> Result<Self, ClosureUsageAuthorityError>
    {
        if end_inclusive.0 <= start_exclusive.0 {
            return Err(ClosureUsageAuthorityError::InvalidUsageWindow { start_exclusive, end_inclusive });
        }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ClosureUsageRequest { id: ClosureUsageRequestId, window: ClosureUsageWindow }
impl ClosureUsageRequest {
    pub const fn new(id: ClosureUsageRequestId, window: ClosureUsageWindow) -> Self { Self { id, window } }
    pub const fn id(self) -> ClosureUsageRequestId { self.id }
    pub const fn window(self) -> ClosureUsageWindow { self.window }
}
...
impl ClosureUsageAuthorityStamp {
    ...
    pub const fn anchor_tick(&self) -> CanonicalTick { self.anchor_tick }
    pub const fn used_through(&self) -> CanonicalTick { self.used_through }
}
...
impl ClosureUsageReceipt {
    pub const fn id(&self) -> ClosureUsageReceiptId { self.id }
    pub const fn request(&self) -> ClosureUsageRequestId { self.request }
    pub const fn source_revision(&self) -> ClosureUsageRevision { self.source_revision }
    pub const fn resulting_revision(&self) -> ClosureUsageRevision { self.resulting_revision }
    pub const fn usage_policy(&self) -> ClosureUsagePolicyKey { self.usage_policy }
    pub const fn anchor_id(&self) -> ClosureValidationAnchorId { self.anchor_id }
    pub const fn anchor_revision(&self) -> ClosureValidationAnchorRevision { self.anchor_revision }
    pub const fn window(&self) -> ClosureUsageWindow { self.window }
...
impl ClosureRevalidationRequirement {
    ...
    pub const fn anchor_revision(&self) -> ClosureValidationAnchorRevision { self.anchor_revision }
    pub const fn anchor_tick(&self) -> CanonicalTick { self.anchor_tick }
    pub const fn qualified_end(&self) -> CanonicalTick { self.qualified_end }
    pub const fn requested_end(&self) -> CanonicalTick { self.requested_end }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PreparedClosureUsage { request: ClosureUsageRequest, source_authority: ClosureUsageAuthorityStamp }
impl PreparedClosureUsage {
    pub const fn request(&self) -> ClosureUsageRequest { self.request }
    pub const fn source_authority(&self) -> &ClosureUsageAuthorityStamp { &self.source_authority }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClosureUsagePreparationOutcome {
    AlreadyCommitted(Box<ClosureUsageReceipt>),
    Ready(Box<PreparedClosureUsage>),
    RevalidationRequired(ClosureRevalidationRequirement),
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct CommittedUsage { request: ClosureUsageRequest, receipt: ClosureUsageReceipt }

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ClosureAnchorInstallRequestId(pub u128);

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClosureAnchorInstallRequest { id: ClosureAnchorInstallRequestId, anchor: ClosureValidationAnchor }
impl ClosureAnchorInstallRequest {
    pub const fn new(id: ClosureAnchorInstallRequestId, anchor: ClosureValidationAnchor) -> Self { Self { id, anchor } }
    pub const fn id(&self) -> ClosureAnchorInstallRequestId { self.id }
    pub const fn anchor(&self) -> &ClosureValidationAnchor { &self.anchor }
}
...
impl ClosureAnchorInstallReceipt {
    pub const fn request(&self) -> ClosureAnchorInstallRequestId { self.request }
    pub const fn source_revision(&self) -> ClosureUsageRevision { self.source_revision }
    pub const fn resulting_revision(&self) -> ClosureUsageRevision { self.resulting_revision }
    pub const fn anchor_id(&self) -> ClosureValidationAnchorId { self.anchor_id }
    pub const fn anchor_revision(&self) -> ClosureValidationAnchorRevision { self.anchor_revision }
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct CommittedAnchorInstall { request: ClosureAnchorInstallRequest, receipt: ClosureAnchorInstallReceipt }
...
impl AuthoritativeClosureUsageState {
    pub fn bootstrap(
        ...
        let policy = registry.definition(anchor.usage_policy())
            .ok_or(ClosureUsageAuthorityError::UnknownUsagePolicy { policy: anchor.usage_policy() })?;
    ...
    pub const fn authority_stamp(&self) -> &ClosureUsageAuthorityStamp { &self.authority }
    pub const fn anchor(&self) -> &ClosureValidationAnchor { &self.anchor }
    ...
    pub fn prepare_usage(
        ...
        if let Some(committed) = self.committed_usage.get(&request.id()) {
            if committed.request == request {
                return Ok(ClosureUsagePreparationOutcome::AlreadyCommitted(Box::new(committed.receipt.clone())));
            }
            return Err(ClosureUsageAuthorityError::ConflictingUsageRequestId { request: request.id() });
        }
        validate_state_context(self, registry, information_policy, closures, acceptances, spatiotemporal)?;
        ...
        let definition = registry.definition(self.authority.usage_policy())
            .ok_or(ClosureUsageAuthorityError::UnknownUsagePolicy { policy: self.authority.usage_policy() })?;
        let qualified_end = self.authority.anchor_tick().0
        ...
        Ok(ClosureUsagePreparationOutcome::Ready(Box::new(PreparedClosureUsage {
            request,
    ...
    pub fn commit_usage(
        ...
        if let Some(committed) = self.committed_usage.get(&prepared.request().id()) {
            if committed.request == prepared.request() { return Ok(committed.receipt.clone()); }
            return Err(ClosureUsageAuthorityError::ConflictingUsageRequestId { request: prepared.request().id() });
        }
        if self.authority != *prepared.source_authority() {
            return Err(ClosureUsageAuthorityError::PreparedUsageStale);
        }
        let ClosureUsagePreparationOutcome::Ready(current) = self.prepare_usage(
            prepared.request(), registry, information_policy, closures, acceptances, spatiotemporal,
        )? else {
            return Err(ClosureUsageAuthorityError::PreparedUsageNoLongerAuthorized);
        };
        if *current != *prepared { return Err(ClosureUsageAuthorityError::PreparedUsageStale); }

        let source_revision = self.authority.revision();
        let resulting_revision = ClosureUsageRevision(source_revision.0.checked_add(1)
            .ok_or(ClosureUsageAuthorityError::UsageRevisionOverflow)?);
        let receipt = ClosureUsageReceipt {
            id: ClosureUsageReceiptId { scope: self.authority.scope(), revision: resulting_revision },
        ...
        self.committed_usage.insert(prepared.request().id(), CommittedUsage {
            request: prepared.request(), receipt: receipt.clone(),
        });
        Ok(receipt)
    }
    ...
    pub fn install_anchor(
        ...
        if let Some(committed) = self.committed_anchor_installs.get(&request.id()) {
            if committed.request == request { return Ok(committed.receipt.clone()); }
            return Err(ClosureUsageAuthorityError::ConflictingAnchorInstallRequestId { request: request.id() });
        }
        validate_state_context(self, registry, information_policy, closures, acceptances, spatiotemporal)?;
        ...
        if request.anchor().usage_policy() != self.authority.usage_policy() {
            return Err(ClosureUsageAuthorityError::AnchorUsagePolicyMismatch {
                expected: self.authority.usage_policy(), actual: request.anchor().usage_policy(),
            });
        }
        if request.anchor().tick().0 < self.authority.used_through().0 {
            return Err(ClosureUsageAuthorityError::AnchorMovesBackward {
                used_through: self.authority.used_through(), anchor_tick: request.anchor().tick(),
            });
        }
        ...
        if self.seen_anchors.contains(&identity) {
            return Err(ClosureUsageAuthorityError::AnchorAlreadyUsed {
                anchor: request.anchor().id(), revision: request.anchor().revision(),
            });
        }
        ...
        let receipt = ClosureAnchorInstallReceipt {
            request: request.id(), source_revision, resulting_revision,
            anchor_id: request.anchor().id(), anchor_revision: request.anchor().revision(),
        ...
        self.committed_anchor_installs.insert(request.id(), CommittedAnchorInstall {
            request, receipt: receipt.clone(),
        });
        Ok(receipt)
    }
}
...
fn validate_anchor(
    ...
    if registry.definition(anchor.usage_policy()).is_none() {
        return Err(ClosureUsageAuthorityError::UnknownUsagePolicy { policy: anchor.usage_policy() });
    }
    Ok(())
}
...
fn validate_state_context(
    ...
    registry.validate_current(information_policy, closures, acceptances, spatiotemporal)?;
...
pub enum ClosureUsageAuthorityError {
    ...
    ConflictingUsagePolicyRegistration { policy: ClosureUsagePolicyKey },
    ...
    ProcessDoesNotPermitClosure { process: ProcessKey, information: EcologicalInformation },
    ...
    AnchorUsagePolicyMismatch { expected: ClosureUsagePolicyKey, actual: ClosureUsagePolicyKey },
    AnchorStateMismatch,
    InvalidUsageWindow { start_exclusive: CanonicalTick, end_inclusive: CanonicalTick },
    NonContiguousUsageWindow { expected_start: CanonicalTick, actual_start: CanonicalTick },
    QualifiedHorizonOverflow,
    ConflictingUsageRequestId { request: ClosureUsageRequestId },
    PreparedUsageStale,
    PreparedUsageNoLongerAuthorized,
    UsageRevisionOverflow,
    ConflictingAnchorInstallRequestId { request: ClosureAnchorInstallRequestId },
    AnchorMovesBackward { used_through: CanonicalTick, anchor_tick: CanonicalTick },
    AnchorAlreadyUsed { anchor: ClosureValidationAnchorId, revision: ClosureValidationAnchorRevision },
}

impl fmt::Display for ClosureUsageAuthorityError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ZeroMaximumUseHorizon => write!(f, "closure usage horizon must be at least one canonical tick"),
            Self::ConflictingUsagePolicyRegistration { policy } => write!(f, "conflicting closure-usage policy registration for {}@{}", policy.id(), policy.version()),
            Self::InformationPolicyIdentity(error) => write!(f, "information-policy identity error: {error}"),
            Self::InformationPolicyAuthorityChanged => write!(f, "exact information-policy authority changed"),
            ...
            Self::ClosureQualificationAuthorityChanged => write!(f, "typed closure qualification authority changed"),
            Self::ClosureAcceptance(error) => write!(f, "typed closure process-acceptance error: {error}"),
            Self::ClosureAcceptanceAuthorityChanged => write!(f, "typed closure process-acceptance authority changed"),
            Self::SpatiotemporalPolicy(error) => write!(f, "spatiotemporal policy error: {error}"),
            Self::SpatiotemporalAuthorityChanged => write!(f, "spatiotemporal authority changed"),
            Self::ProcessDoesNotPermitClosure { process, information } => write!(f, "process {process:?} does not permit closure evidence for {information:?}"),
            Self::UsageHorizonExceedsQualification { policy, maximum_use, qualified_horizon } => write!(f, "closure-usage policy {}@{} requests {} ticks but qualification covers only {} ticks", policy.id(), policy.version(), maximum_use.get(), qualified_horizon),
            Self::RepresentationDoesNotCarryClosureLineage { representation, information, lineage } => write!(f, "representation {representation:?} does not carry closure lineage {} for {information:?}", lineage.0),
            ...
            Self::AnchorPolicyAuthorityChanged => write!(f, "closure validation anchor was minted under different usage-policy authority"),
            Self::AnchorUsagePolicyMismatch { expected, actual } => write!(f, "closure anchor policy mismatch: expected {}@{}, got {}@{}", expected.id(), expected.version(), actual.id(), actual.version()),
            Self::AnchorStateMismatch => write!(f, "canonical closure anchor state does not match its usage authority stamp"),
            Self::InvalidUsageWindow { start_exclusive, end_inclusive } => write!(f, "closure usage window must advance canonical time: ({}, {}]", start_exclusive.0, end_inclusive.0),
            Self::NonContiguousUsageWindow { expected_start, actual_start } => write!(f, "closure usage must continue from authoritative tick {}; request starts at {}", expected_start.0, actual_start.0),
            Self::QualifiedHorizonOverflow => write!(f, "closure validation anchor plus qualified horizon overflows canonical tick"),
            Self::ConflictingUsageRequestId { request } => write!(f, "closure usage request id {} was reused with conflicting semantics", request.0),
            Self::PreparedUsageStale => write!(f, "prepared closure usage is stale against current usage authority"),
            Self::PreparedUsageNoLongerAuthorized => write!(f, "prepared closure usage is no longer inside the qualified anchor horizon"),
            Self::UsageRevisionOverflow => write!(f, "closure-usage revision overflow"),
            Self::ConflictingAnchorInstallRequestId { request } => write!(f, "closure anchor-install request id {} was reused with conflicting semantics", request.0),
            Self::AnchorMovesBackward { used_through, anchor_tick } => write!(f, "new closure validation anchor at tick {} precedes already authorized use through tick {}", anchor_tick.0, used_through.0),
            Self::AnchorAlreadyUsed { anchor, revision } => write!(f, "closure validation anchor {} revision {} has already been used by this authority scope", anchor.0, revision.0),
        }
    }
}
...
mod tests {
    ...
    const CLOSURE_REGISTRY: TypedClosureQualificationRegistryKey = TypedClosureQualificationRegistryKey::new(30_050, 1);
    const ACCEPTANCE_REGISTRY: TypedClosureProcessAcceptanceRegistryKey = TypedClosureProcessAcceptanceRegistryKey::new(30_060, 1);
    const ST_REGISTRY: SpatiotemporalPolicyRegistryKey = SpatiotemporalPolicyRegistryKey::new(30_070, 1);
    const USAGE_REGISTRY: ClosureUsagePolicyRegistryKey = ClosureUsagePolicyRegistryKey::new(30_080, 1);
    ...
    fn evidence() -> QualifiedClosureEvidence {
        QualifiedClosureEvidence::new(ClosureModelVersion(1), ClosureDomainToken(2), ErrorPpm::new(10_000).unwrap(), LINEAGE)
    }

    fn information_policy() -> InformationPolicyRegistry {
        let mut builder = InformationPolicyRegistryBuilder::new(INFO_POLICY);
        builder.register_closure_evidence(RegisteredClosureEvidence::new(evidence(), ClosureEvidenceStatus::Qualified)).unwrap();
        builder.register_process(ProcessInformationProfile::new(
            PROCESS,
            EcologicalAuthorityLevel::Coarse,
            [ProcessInformationRequirement::closure_allowed(
                EcologicalInformation::OccupancyDistribution,
                ClosureAcceptance::new(ErrorPpm::new(20_000).unwrap(), Some(ClosureModelVersion(1)), Some(ClosureDomainToken(2))),
            )],
        )).unwrap();
        builder.register_representation(RepresentationCapabilities::new(
            REPRESENTATION,
            EcologicalAuthorityLevel::Coarse,
            [(EcologicalInformation::OccupancyDistribution, CapabilityEvidence::QualifiedClosure(evidence()))],
        )).unwrap();
        builder.seal()
    }

    fn closures(policy: &ManifestBoundInformationPolicyRegistry<'_>, horizon: u64) -> TypedClosureQualificationRegistry {
        let mut builder = TypedClosureQualificationRegistryBuilder::new(CLOSURE_REGISTRY);
        builder.register(TypedClosureQualification::new(
            evidence(), OBSERVABLE,
            ClosureErrorMetric::MaximumRelativePpm { zero_reference: RelativeZeroReferencePolicy::RejectZeroReference },
    ...
    fn acceptances(policy: &ManifestBoundInformationPolicyRegistry<'_>) -> TypedClosureProcessAcceptanceRegistry {
        let mut builder = TypedClosureProcessAcceptanceRegistryBuilder::new(ACCEPTANCE_REGISTRY);
        builder.register(TypedClosureProcessAcceptance::new(
            PROCESS, EcologicalInformation::OccupancyDistribution, OBSERVABLE,
            ClosureErrorMetric::MaximumRelativePpm { zero_reference: RelativeZeroReferencePolicy::RejectZeroReference },
            ErrorPpm::new(10_000).unwrap(), ClosureEvaluationHorizonTicks::new(50).unwrap(),
            ClosureAggregationSemantics::MaximumOverHorizon,
            Some(ClosureModelVersion(1)), Some(ClosureDomainToken(2)),
        )).unwrap();
        builder.seal(policy).unwrap()
    }

    fn spatiotemporal(policy: &ManifestBoundInformationPolicyRegistry<'_>) -> SpatiotemporalPolicyRegistry {
        let mut builder = SpatiotemporalPolicyRegistryBuilder::new(ST_REGISTRY);
        builder.register_process_requirement(ProcessSpatiotemporalRequirement::new(
            PROCESS, EcologicalInformation::OccupancyDistribution,
            SpatialResolutionUnits::new(5).unwrap(), MaximumStateAgeTicks(2),
            MaximumUpdateIntervalTicks::new(1).unwrap(), IntegrationSemantics::Instantaneous,
            None, None,
        ).unwrap()).unwrap();
        builder.register_representation_capability(RepresentationSpatiotemporalCapability::new(
            REPRESENTATION, EcologicalInformation::OccupancyDistribution,
            SpatialResolutionUnits::new(5).unwrap(), MaximumUpdateIntervalTicks::new(1).unwrap(),
            IntegrationSemantics::Instantaneous, None, None,
        ).unwrap()).unwrap();
        builder.seal(policy).unwrap()
    }
    ...
    fn fixture(maximum_use: u64, qualification_horizon: u64) -> Fixture {
        ...
        builder.register(ClosureUsagePolicyDefinition::new(
            USAGE_POLICY, PROCESS, EcologicalInformation::OccupancyDistribution,
            LINEAGE, REPRESENTATION, MaximumClosureUseTicks::new(maximum_use).unwrap(),
        )).unwrap();
        let usage = builder.seal(&bound, &closures, &acceptances, &spatiotemporal).unwrap();
        Fixture { information_policy, closures, acceptances, spatiotemporal, usage }
    }

    fn anchor(usage: &ClosureUsagePolicyRegistry, id: u128, revision: u64, tick: u64) -> ClosureValidationAnchor {
    ...
    fn window(start: u64, end: u64) -> ClosureUsageWindow {
        ClosureUsageWindow::new(CanonicalTick(start), CanonicalTick(end)).unwrap()
    }
    fn ready(state: &AuthoritativeClosureUsageState, fixture: &Fixture, request: ClosureUsageRequest) -> PreparedClosureUsage {
        let bound = bound(fixture);
        let ClosureUsagePreparationOutcome::Ready(prepared) = state.prepare_usage(
            request, &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap() else { panic!("expected ready closure usage"); };
        *prepared
    }
    fn commit(state: &mut AuthoritativeClosureUsageState, fixture: &Fixture, prepared: &PreparedClosureUsage) -> ClosureUsageReceipt {
        let bound = bound(fixture);
        state.commit_usage(prepared, &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal).unwrap()
    }

    #[test]
    fn split_windows_cannot_extend_anchor_horizon() {
        let fixture = fixture(100, 100);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let first = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 40)));
        commit(&mut state, &fixture, &first);
        let second = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(2), window(40, 100)));
        commit(&mut state, &fixture, &second);
        let bound = bound(&fixture);
        let outcome = state.prepare_usage(
            ClosureUsageRequest::new(ClosureUsageRequestId(3), window(100, 101)),
            &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap();
    ...
    #[test]
    fn tiny_requests_do_not_reset_horizon() {
        let fixture = fixture(10, 10);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 50)).unwrap();
        for offset in 0_u64..10 {
            let prepared = ready(&state, &fixture, ClosureUsageRequest::new(
                ClosureUsageRequestId(u128::from(offset + 1)), window(50 + offset, 51 + offset),
            ));
            commit(&mut state, &fixture, &prepared);
        }
        let bound = bound(&fixture);
        assert!(matches!(state.prepare_usage(
            ClosureUsageRequest::new(ClosureUsageRequestId(99), window(60, 61)),
            &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap(), ClosureUsagePreparationOutcome::RevalidationRequired(_)));
    }

    #[test]
    fn acknowledgement_loss_retry_is_idempotent() {
        let fixture = fixture(100, 100);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let request = ClosureUsageRequest::new(ClosureUsageRequestId(7), window(0, 20));
        let prepared = ready(&state, &fixture, request);
    ...
    #[test]
    fn competing_preparations_linearize_once() {
        let fixture = fixture(100, 100);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let first = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 10)));
        let second = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(2), window(0, 20)));
        commit(&mut state, &fixture, &first);
        let bound = bound(&fixture);
        assert!(matches!(state.commit_usage(
            &second, &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ), Err(ClosureUsageAuthorityError::PreparedUsageStale)));
        assert_eq!(state.authority_stamp().used_through(), CanonicalTick(10));
    }

    #[test]
    fn exhausted_request_has_zero_mutation() {
        let fixture = fixture(10, 10);
        let state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let before = state.clone();
        let bound = bound(&fixture);
        let outcome = state.prepare_usage(
            ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 11)),
            &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap();
        assert!(matches!(outcome, ClosureUsagePreparationOutcome::RevalidationRequired(_)));
        assert_eq!(state, before);
    }

    #[test]
    fn explicit_new_anchor_resets_horizon_but_retry_does_not() {
        let fixture = fixture(10, 10);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let prepared = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 10)));
        commit(&mut state, &fixture, &prepared);
        let bound = bound(&fixture);
        let install = ClosureAnchorInstallRequest::new(ClosureAnchorInstallRequestId(2), anchor(&fixture.usage, 2, 0, 10));
        let installed = state.install_anchor(
            install.clone(), &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap();
        assert_eq!(installed.resulting_revision(), ClosureUsageRevision(2));
        let next = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(3), window(10, 20)));
        commit(&mut state, &fixture, &next);
        let current_revision = state.authority_stamp().revision();
        let retry = state.install_anchor(
            install, &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
        ).unwrap();
        assert_eq!(retry.resulting_revision(), ClosureUsageRevision(2));
        assert_eq!(state.authority_stamp().revision(), current_revision);
        assert_eq!(state.authority_stamp().used_through(), CanonicalTick(20));
    }

    #[test]
    fn old_anchor_identity_cannot_be_reused() {
        let fixture = fixture(10, 10);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let prepared = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 5)));
        commit(&mut state, &fixture, &prepared);
        let before = state.clone();
        let bound = bound(&fixture);
        assert!(matches!(state.install_anchor(
            ClosureAnchorInstallRequest::new(ClosureAnchorInstallRequestId(2), anchor(&fixture.usage, 1, 0, 5)),
            &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
    ...
    #[test]
    fn policy_cannot_exceed_qualification_horizon() {
        let information_policy = information_policy();
        let bound = ManifestBoundInformationPolicyRegistry::new(&information_policy);
        ...
        builder.register(ClosureUsagePolicyDefinition::new(
            USAGE_POLICY, PROCESS, EcologicalInformation::OccupancyDistribution,
            LINEAGE, REPRESENTATION, MaximumClosureUseTicks::new(11).unwrap(),
        )).unwrap();
        assert!(matches!(builder.seal(&bound, &closures, &acceptances, &spatiotemporal),
            Err(ClosureUsageAuthorityError::UsageHorizonExceedsQualification { .. })));
    }

    #[test]
    fn changed_exact_closure_corpus_stales_usage() {
        let fixture = fixture(100, 100);
        let state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 0)).unwrap();
        let bound = bound(&fixture);
        let changed_closures = closures(&bound, 101);
        assert!(matches!(state.prepare_usage(
            ClosureUsageRequest::new(ClosureUsageRequestId(1), window(0, 1)),
            &fixture.usage, &bound, &changed_closures, &fixture.acceptances, &fixture.spatiotemporal,
        ), Err(ClosureUsageAuthorityError::ClosureQualificationAuthorityChanged)));
    }

    #[test]
    fn backward_anchor_is_rejected_without_mutation() {
        let fixture = fixture(100, 100);
        let mut state = AuthoritativeClosureUsageState::bootstrap(SCOPE, &fixture.usage, anchor(&fixture.usage, 1, 0, 50)).unwrap();
        let prepared = ready(&state, &fixture, ClosureUsageRequest::new(ClosureUsageRequestId(1), window(50, 60)));
        commit(&mut state, &fixture, &prepared);
        let before = state.clone();
        let bound = bound(&fixture);
        assert!(matches!(state.install_anchor(
            ClosureAnchorInstallRequest::new(ClosureAnchorInstallRequestId(2), anchor(&fixture.usage, 2, 0, 59)),
            &fixture.usage, &bound, &fixture.closures, &fixture.acceptances, &fixture.spatiotemporal,
