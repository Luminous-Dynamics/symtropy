...
/// Fields are constructor-sealed so a validated schema/profile/digest tuple
...
impl TypedAuthorityRefV1 {
    ...
    pub fn profile(&self) -> &SemanticId {
        &self.profile
    }
...
impl ProvenanceBindingV1 {
    ...
    pub fn claim_id(&self) -> &SemanticId {
        &self.claim_id
    }

    pub const fn class(&self) -> ProvenanceClass {
        self.class
    }

    pub fn source(&self) -> &TypedAuthorityRefV1 {
        &self.source
    }
...
/// from semantic identity and semantic equality. Semantic fields are private so
/// cached identity cannot diverge through post-construction mutation.
#[derive(Debug, Clone)]
...
impl PartialEq for ScenarioTemplateV1 {
    fn eq(&self, other: &Self) -> bool {
        self.template_id == other.template_id
            && self.genesis_profile == other.genesis_profile
...
impl Eq for ScenarioTemplateV1 {}
...
impl ScenarioTemplateV1 {
    ...
    pub fn template_id(&self) -> &SemanticId {
        &self.template_id
    }

    pub fn genesis_profile(&self) -> &SemanticId {
...
/// Semantic fields are private so the cached digest remains constructor-bound.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl GenesisRequestV1 {
    ...
    pub fn request_id(&self) -> &SemanticId {
        &self.request_id
    }

    pub const fn template_digest(&self) -> CanonicalDigest {
        self.template_digest
    }

    pub const fn intent(&self) -> GenesisIntentV1 {
        self.intent
    }
...
/// Pure, complete candidate assembled before admission. Fields are externally
/// immutable so the candidate digest cannot diverge from its bound components.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ScenarioGenesisCandidateV1 {
    template_digest: CanonicalDigest,
    request_digest: CanonicalDigest,
    lineage: WorldLineageRefV1,
    candidate_digest: CanonicalDigest,
}

impl ScenarioGenesisCandidateV1 {
    pub const fn template_digest(&self) -> CanonicalDigest {
        self.template_digest
    }
    ...
    pub const fn lineage(&self) -> WorldLineageRefV1 {
        self.lineage
    }

    pub const fn candidate_digest(&self) -> CanonicalDigest {
        self.candidate_digest
    }
}

/// Immutable successful admission evidence. Fields are externally immutable;
/// callers can inspect them only through read-only accessors.
#[derive(Debug, Clone, PartialEq, Eq)]
