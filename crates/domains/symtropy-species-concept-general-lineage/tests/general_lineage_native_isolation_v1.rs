...
mod isolation_fixture {
    ...
    pub(super) fn with_supported(
        ...
        let current_a =
            iso_current_study(&a, &a_study, &auth, ctx, ContactKind::Barrier, 40);
        let current_b =
            iso_current_study(&b, &b_study, &auth, ctx, ContactKind::Barrier, 60);
        ...
        .unwrap();
        assert_eq!(validated.evidence().status, ReproductiveIsolationStatus::Supported);
        f(&validated);
    }
}
