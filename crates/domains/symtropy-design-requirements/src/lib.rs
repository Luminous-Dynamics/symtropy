    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedSchemaVersion(version) => {
                write!(
                    formatter,
                    "unsupported requirement-set schema version {version}"
                )
            }
            ...
            Self::DerivationCycle => {
                write!(formatter, "requirement derivation graph contains a cycle")
            }
            ...
            Self::NonCanonicalRequirementOrder => {
                write!(
                    formatter,
                    "requirements are not in canonical identity order"
                )
            }
...
mod tests {
    use super::*;
    use symtropy_design::{ContentDigest, DesignArtifactId};
    ...
    fn derivation_cycle_is_rejected() {
        let a = requirement("requirement:a", vec![req_id("requirement:b")], Vec::new());
        let b = requirement("requirement:b", vec![req_id("requirement:a")], Vec::new());
        let result = RequirementSet::new(
            RequirementSetId::new(id("requirement-set:test")),
            1,
            vec![a, b],
    ...
    fn topological_order_is_deterministic_parent_before_child() {
        ...
        let b = requirement("requirement:b", vec![root.id.clone()], Vec::new());
        let a = requirement("requirement:a", vec![root.id.clone()], Vec::new());
