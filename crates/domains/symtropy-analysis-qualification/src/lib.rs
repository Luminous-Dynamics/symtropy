        ...
        for assessment in &self.assessments {
                ...
                && previous >= &assessment.facet_id
            {
                return if previous == &assessment.facet_id {
                    ...
                    Err(QualificationError::NonCanonicalOrder("cut.assessments"))
                };
            }
...
fn encode_digest(bytes: &mut Vec<u8>, digest: &ContentDigest) -> Result<(), QualificationError> {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedProfileSchema(version) => {
                write!(
                    formatter,
                    "unsupported qualification profile schema {version}"
                )
            }
            Self::UnsupportedCutSchema(version) => {
                write!(formatter, "unsupported qualification cut schema {version}")
            }
            Self::FacetRulesRequired => write!(
                formatter,
                "qualification profile requires at least one facet rule"
            ),
            Self::DuplicateFacet(id) => write!(formatter, "duplicate qualification facet {id}"),
            Self::DuplicateAssessment(id) => write!(
                formatter,
                "duplicate assessment for qualification facet {id}"
            ),
            Self::DuplicateSupportingEvidence(reference) => {
                write!(
                    formatter,
                    "duplicate supporting qualification evidence {reference:?}"
                )
            }
            Self::UnknownFacet(id) => write!(
                formatter,
                "assessment references unknown qualification facet {id}"
            ),
            Self::WrongFacetAuthority {
                facet_id,
                expected,
                actual,
            } => write!(
            ...
            Self::RequestMismatch => write!(
                formatter,
                "qualification cut does not bind the supplied exact analysis request"
            ),
            Self::EvidenceMismatch => write!(
                formatter,
                "qualification cut does not bind the supplied exact analysis evidence"
            ),
            Self::ProfileMismatch => write!(
                formatter,
                "qualification cut does not bind the supplied exact qualification profile"
            ),
            Self::NonCanonicalOrder(field) => write!(
                formatter,
                "qualification field {field} is not canonically ordered"
            ),
...
mod tests {
    ...
    fn request() -> AnalysisRequest {
        let subject =
            DesignRevisionRef::new(DesignId::new(id("design:bracket")), 1, digest("design-a"))
                .unwrap();
    ...
    fn exact_solver_evidence_substitution_changes_cut_identity() {
        ...
        assert_ne!(
            cut_digest(&left, &request, &left_evidence, &profile).unwrap(),
            cut_digest(&right, &request, &right_evidence, &profile).unwrap()
        );
    }
