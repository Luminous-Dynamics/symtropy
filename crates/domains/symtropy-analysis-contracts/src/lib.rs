...
impl ExactSemanticRef {
    ...
    pub fn validate(&self) -> Result<(), AnalysisError> {
        ...
        self.content_digest.validate().map_err(AnalysisError::Design)
    }
}
...
impl SolverIdentity {
    ...
    fn validate(&self) -> Result<(), AnalysisError> {
        ...
        if self.version.is_empty() || self.version.len() > 128 || self.version.chars().any(char::is_control) {
...
impl AnalysisEvidence {
    ...
    pub fn exact_ref(&self, request: &AnalysisRequest) -> Result<AnalysisEvidenceRef, AnalysisError> {
    ...
    pub fn content_digest(&self, request: &AnalysisRequest) -> Result<ContentDigest, AnalysisError> {
    ...
    pub fn validate_against(&self, request: &AnalysisRequest) -> Result<(), AnalysisError> {
        if self.schema_version != ANALYSIS_EVIDENCE_SCHEMA_VERSION {
            return Err(AnalysisError::UnsupportedEvidenceSchema(self.schema_version));
        }
        validate_stable_id(self.id.stable_id())?;
        request.validate()?;
...
fn validate_unique_observation_ids(observations: &[AnalysisObservation]) -> Result<(), AnalysisError> {
...
fn encode_design_ref(bytes: &mut Vec<u8>, reference: &DesignRevisionRef) -> Result<(), AnalysisError> {
...
fn encode_exact_ref(bytes: &mut Vec<u8>, reference: &ExactSemanticRef) -> Result<(), AnalysisError> {
...
fn encode_artifacts(bytes: &mut Vec<u8>, artifacts: &[AnalysisArtifactRef]) -> Result<(), AnalysisError> {
...
pub enum AnalysisError {
    ...
    ObservableContractMismatch { observable_id: StableId },
    LengthOverflow,
...
impl fmt::Display for AnalysisError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidSolverVersion(version) => write!(formatter, "invalid solver version: {version:?}"),
            Self::UnsupportedRequestSchema(version) => write!(formatter, "unsupported analysis request schema {version}"),
            Self::UnsupportedEvidenceSchema(version) => write!(formatter, "unsupported analysis evidence schema {version}"),
            Self::DuplicateArtifact(id) => write!(formatter, "duplicate analysis artifact {id}"),
            Self::DuplicateObservable(id) => write!(formatter, "duplicate requested observable {id}"),
            Self::DuplicateExactSemanticRef(reference) => write!(formatter, "duplicate exact semantic reference {reference:?}"),
            Self::DuplicateObservation(id) => write!(formatter, "duplicate analysis observation {id}"),
            Self::NonCanonicalOrder(field) => write!(formatter, "analysis field {field} is not canonically ordered"),
            Self::InvalidMeasurementInterval { lower, upper } => write!(formatter, "analysis measurement interval is invalid: {lower}..{upper}"),
            Self::ZeroMeasurementResolution => write!(formatter, "analysis measurement resolution must be non-zero"),
            Self::RequestIdentityMismatch => write!(formatter, "analysis evidence does not bind the supplied exact request"),
            Self::DesignSubjectMismatch => write!(formatter, "analysis evidence design subject differs from the request subject"),
            Self::ConvergenceOnIncompleteRun => write!(formatter, "an incomplete analysis run cannot claim numerical convergence"),
            ...
            Self::UnrequestedResultObservable(id) => write!(formatter, "analysis emitted unrequested result observable {id}"),
            Self::ObservableContractMismatch { observable_id } => write!(formatter, "analysis result {observable_id} changed requested dimension/unit semantics"),
...
mod tests {
    ...
    fn request_for(subject: DesignRevisionRef, observables: Vec<ObservableRequest>) -> AnalysisRequest {
        AnalysisRequest::new(
    ...
    fn evidence(
        ...
        AnalysisEvidence::new(
            ...
            exact("authority:qualification", "applicability:bracket-static", "applicability"),
            run,
            numerical,
            Vec::new(),
            observations,
    ...
    fn failed_run_cannot_claim_convergence() {
        ...
        assert!(matches!(result, Err(AnalysisError::ConvergenceOnIncompleteRun)));
    }
