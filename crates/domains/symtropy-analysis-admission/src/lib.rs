...
impl QualificationAdmissionPolicy {
    ...
    pub fn validate_against(&self, profile: &QualificationProfile) -> Result<(), AdmissionError> {
        self.validate_structure()?;
...
impl AdmittedAnalysisQualification {
    ...
    pub fn validate_current(
        ...
        let reproduced =
            build_admission(self.id.clone(), evidence, profile, cut, policy, registry)?;
        if &reproduced == self {
            Ok(())
        } else {
...
fn build_admission(
    ...
    if decision.overall != QualificationOutcome::Established {
        return Err(AdmissionError::QualificationNotEstablished(
            decision.overall,
        ));
    }
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedVerificationRegistrySchema(version) => {
                write!(
                    formatter,
                    "unsupported verification registry schema {version}"
                )
            }
            Self::UnsupportedAdmissionReceiptSchema(version) => {
                write!(formatter, "unsupported admission receipt schema {version}")
            }
            Self::FacetPolicyRequired => write!(
                formatter,
                "admission policy requires at least one facet rule"
            ),
            Self::DuplicateFacetPolicy(id) => {
                write!(formatter, "duplicate admission facet policy {id}")
            }
            ...
            Self::MissingFacetPolicy(id) => {
                write!(formatter, "missing admission policy for facet {id}")
            }
            ...
            Self::QualificationProfileMismatch => write!(
                formatter,
                "admission policy does not bind the supplied exact A1 profile"
            ),
            Self::AdmissionPolicyMismatch => write!(
                formatter,
                "verification registry does not bind the supplied exact admission policy"
            ),
            Self::UnknownFacet(id) => write!(
                formatter,
                "verification record references unknown facet {id}"
            ),
            Self::IssuerProfileMismatch(id) => write!(
                formatter,
                "verification record changed exact issuer profile for facet {id}"
            ),
            Self::VerifierProfileMismatch(id) => write!(
                formatter,
                "verification record changed exact verifier profile for facet {id}"
            ),
            Self::DuplicateVerificationRecord {
                cut_digest,
                facet_id,
            } => write!(
                formatter,
                "duplicate verification record for cut {}:{} facet {facet_id}",
                cut_digest.algorithm, cut_digest.value
            ),
            Self::QualificationNotEstablished(outcome) => write!(
                formatter,
                "A1 qualification is not established: {outcome:?}"
            ),
            Self::MissingAssessment(id) => write!(
                formatter,
                "established A1 cut unexpectedly lacks facet assessment {id}"
            ),
            Self::VerificationRequired(id) => write!(
                formatter,
                "facet {id} has no exact admitted verification record"
            ),
            Self::VerificationNotCurrent { facet_id, status } => write!(
                formatter,
                "facet {facet_id} verification status is {status:?}, not Verified"
            ),
            Self::VerificationFacetMismatch(id) => write!(
                formatter,
                "verification record facet does not match A1 facet {id}"
            ),
            Self::VerificationCutMismatch(id) => write!(
                formatter,
                "verification record does not bind exact A1 cut for facet {id}"
            ),
            Self::VerificationSubjectMismatch(id) => write!(
                formatter,
                "verification record does not bind exact A0 evidence for facet {id}"
            ),
            Self::AssessmentAttestationMismatch(id) => write!(
                formatter,
                "verification record does not bind exact A1 attestation for facet {id}"
            ),
            Self::AdmissionReceiptStale => write!(
                formatter,
                "admitted qualification no longer matches current exact inputs"
            ),
            Self::NonCanonicalOrder(field) => write!(
                formatter,
                "admission field {field} is not canonically ordered"
            ),
...
mod tests {
    ...
    fn request() -> AnalysisRequest {
        let design =
            DesignRevisionRef::new(DesignId::new(id("design:bracket")), 1, digest("design"))
                .unwrap();
        ...
        cut.assessments()
            ...
            .map(|assessment| {
                ...
                FacetVerificationInput {
                    id: VerificationRecordId::new(id(&format!(
                        "verification:{}",
                        assessment.facet_id.as_str()
                    ))),
    ...
    fn raw_established_a1_cut_is_not_admitted_without_verification() {
        ...
        assert!(matches!(
            result,
            Err(AdmissionError::VerificationRequired(_))
        ));
    }
    ...
    fn verifier_profile_drift_is_rejected_when_registry_is_sealed() {
        ...
        records[0].verifier_profile =
            exact("authority:xenia", "verifier-profile:changed", "changed");
