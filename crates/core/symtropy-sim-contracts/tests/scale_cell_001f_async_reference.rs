...
//! SCALE-CELL-001F async/prewarm reference fixture, revision 3.
//!
...
//! / executor choice / diagnostics != canonical world authority.
...
const SCALE_CELL_001F_SCHEMA_VERSION: u32 = 3;
const MAX_PENDING_HYDRATIONS: usize = 64;
const MAX_COMPLETION_TRACE: usize = 64;
const RUNTIME_EPOCH_DOMAIN: &str = "symtropy.scale-cell-001f.runtime-epoch.v3";
...
impl CanonicalCellState {
    ...
    fn commitment(&self) -> Result<TypedDigest32, RefError> {
        ...
        bytes.extend_from_slice(b"symtropy.scale-cell-001f.canonical-cell.v3\0");
        ...
        Ok(TypedDigest32::sha256(
            "symtropy.scale-cell-001f.canonical-cell.identity.v3",
            SCALE_CELL_001F_SCHEMA_VERSION,
            &bytes,
        )?)
    }
...
impl HydrationRequest {
    fn validate(&self) -> Result<(), RefError> {
        validate_runtime_epoch(&self.runtime_epoch)?;
    ...
    fn lifecycle_identity(&self) -> Result<TypedDigest32, RefError> {
        self.validate()?;
        let mut bytes = Vec::new();
        bytes.extend_from_slice(b"symtropy.scale-cell-001f.hydration-request.v3\0");
        ...
        Ok(TypedDigest32::sha256(
            "symtropy.scale-cell-001f.hydration-request.identity.v3",
            SCALE_CELL_001F_SCHEMA_VERSION,
            &bytes,
        )?)
    }
...
impl HydrationArtifact {
    ...
    fn execution_receipt(&self) -> Result<TypedDigest32, RefError> {
        self.validate()?;
        let mut bytes = Vec::new();
        bytes.extend_from_slice(b"symtropy.scale-cell-001f.hydration-completion.v3\0");
        push_digest(&mut bytes, &self.request.lifecycle_identity()?);
        push_digest(&mut bytes, &self.content);
        Ok(TypedDigest32::sha256(
            "symtropy.scale-cell-001f.hydration-completion.identity.v3",
            SCALE_CELL_001F_SCHEMA_VERSION,
            &bytes,
        )?)
    }
...
fn hydration_content_digest(request: &HydrationRequest) -> Result<TypedDigest32, RefError> {
    request.validate()?;
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"symtropy.scale-cell-001f.hydrated-view.v3\0");
    ...
    Ok(TypedDigest32::sha256(
        "symtropy.scale-cell-001f.hydrated-view.content.v3",
        SCALE_CELL_001F_SCHEMA_VERSION,
        &bytes,
    )?)
}
...
struct ObservationRuntime {
    ...
    completion_attempt_count: u64,
    completion_trace: VecDeque<TypedDigest32>,
}

impl ObservationRuntime {
    fn new(epoch: TypedDigest32) -> Result<Self, RefError> {
        validate_runtime_epoch(&epoch)?;
        Ok(Self {
            epoch,
            next_request_id: 1,
    ...
    fn request_prewarm(
        ...
        let previous_is_pending =
            previous_desired.is_some_and(|request_id| self.pending.contains_key(&request_id));
    ...
    fn reconcile_pending(&mut self, current: &CanonicalCellState) -> Result<usize, RefError> {
        ...
        for request_id in &stale_ids {
            if let Some(pending) = self.pending.remove(request_id) {
                self.remove_desired_if_exact(&pending.request.scope, *request_id);
            }
        }
        Ok(stale_ids.len())
    }
    ...
    fn complete(
        ...
        self.record_completion_attempt(lifecycle_identity);
    ...
    fn retire_exact_pending(&mut self, request: &HydrationRequest) -> Result<bool, RefError> {
        ...
        self.remove_desired_if_exact(&request.scope, request.request_id);
        Ok(true)
    }

    fn remove_desired_if_exact(&mut self, scope: &ScopeId, request_id: u64) {
        if self
            .desired_request_by_scope
            .get(scope)
            .is_some_and(|desired| *desired == request_id)
        {
            self.desired_request_by_scope.remove(scope);
        }
    }

    fn record_completion_attempt(&mut self, lifecycle_identity: TypedDigest32) {
        self.completion_attempt_count = self.completion_attempt_count.saturating_add(1);
        if self.completion_trace.len() >= MAX_COMPLETION_TRACE {
            self.completion_trace.pop_front();
        }
        self.completion_trace.push_back(lifecycle_identity);
    }
    ...
    fn installed_digest(&self) -> Result<TypedDigest32, RefError> {
        let mut bytes = Vec::new();
        bytes.extend_from_slice(b"symtropy.scale-cell-001f.installed-views.v3\0");
        ...
        Ok(TypedDigest32::sha256(
            "symtropy.scale-cell-001f.installed-views.identity.v3",
            SCALE_CELL_001F_SCHEMA_VERSION,
            &bytes,
        )?)
    }
...
fn validate_runtime_epoch(epoch: &TypedDigest32) -> Result<(), RefError> {
    epoch.validate()?;
    if epoch.domain != RUNTIME_EPOCH_DOMAIN
        || epoch.algorithm != DigestAlgorithm::Sha256
        || epoch.schema_version != SCALE_CELL_001F_SCHEMA_VERSION
    {
        return Err(RefError::InvalidRuntimeEpochProfile);
    }
    Ok(())
}
...
fn fixture() -> CanonicalCellState {
    CanonicalCellState {
        ...
        exact_total: 100,
        process_counter: 11,
        prior_scale_context: TypedDigest32::sha256(
...
fn recipe(value: &str) -> TypedDigest32 {
    TypedDigest32::sha256(
        "symtropy.scale-cell-001f.view-recipe.v3",
        SCALE_CELL_001F_SCHEMA_VERSION,
        value.as_bytes(),
    )
    .unwrap()
...
#[test]
fn completion_order_alone_cannot_change_semantic_views() {
    let cell = fixture();
    let scopes = known_scopes();
    let targets = ["body:a", "structure:s0", "micro:m0"];
    let mut prepared = ObservationRuntime::new(runtime_epoch("order-fixture")).unwrap();
    ...
    for target in targets {
        let request = prepared
            ...
            .unwrap();
        artifacts.push(HydrationArtifact::build(request).unwrap());
    }

    let mut ordered = prepared.clone();
    for artifact in artifacts.iter().cloned() {
        ordered.complete(&cell, artifact).unwrap();
    }

    let mut reversed = prepared;
    for artifact in artifacts.into_iter().rev() {
        reversed.complete(&cell, artifact).unwrap();
    }

    assert_eq!(ordered.installed_digest().unwrap(), reversed.installed_digest().unwrap());
    assert_ne!(ordered.completion_trace, reversed.completion_trace);
    assert_eq!(ordered.completion_attempt_count, 3);
    assert_eq!(reversed.completion_attempt_count, 3);
}
...
#[test]
fn same_epoch_forged_wrong_world_cannot_retire_real_pending_work() {
    let cell = fixture();
    let scopes = known_scopes();
    let mut runtime = ObservationRuntime::new(runtime_epoch("same-epoch-runtime")).unwrap();
    let request = runtime
        .request_prewarm(
            &cell,
            &scopes,
    ...
    forged.world = WorldInstanceId::parse("world:foreign").unwrap();
    let forged_artifact = HydrationArtifact::build(forged).unwrap();
    assert_eq!(
        runtime.complete(&cell, forged_artifact),
        Err(RefError::WrongWorld)
    );
    assert_eq!(runtime.pending.len(), 1);
    assert_eq!(
...
fn stale_pending_work_is_reconciled_after_canonical_advance() {
    ...
    assert!(runtime.pending.is_empty());
    assert!(!runtime.desired_request_by_scope.contains_key(&target));
}

#[test]
fn direct_stale_completion_retires_pending_and_desired_together() {
    let cell = fixture();
    let scopes = known_scopes();
    let target = ScopeId::parse("region:r0").unwrap();
    let mut runtime = ObservationRuntime::new(runtime_epoch("stale-complete-runtime")).unwrap();
    let request = runtime
        .request_prewarm(
            &cell,
            &scopes,
...
#[test]
fn diagnostic_completion_trace_is_bounded_and_non_authoritative() {
    let cell = fixture();
    let scopes = known_scopes();
    let mut current_runtime = ObservationRuntime::new(runtime_epoch("trace-current")).unwrap();
    let before = cell.commitment().unwrap();

    let mut foreign_runtime = ObservationRuntime::new(runtime_epoch("trace-foreign")).unwrap();
    let foreign_request = foreign_runtime
        .request_prewarm(
            &cell,
            &scopes,
            ScopeId::parse("micro:m0").unwrap(),
            recipe("foreign-trace"),
            ExecutionProfile::ScaleRuntimeParallelV1,
        )
        .unwrap();
    let foreign_artifact = HydrationArtifact::build(foreign_request).unwrap();

    let attempts = MAX_COMPLETION_TRACE * 2 + 7;
    for _ in 0..attempts {
        assert_eq!(
            current_runtime.complete(&cell, foreign_artifact.clone()),
            Err(RefError::WrongRuntimeEpoch)
        );
    }

    assert_eq!(current_runtime.completion_attempt_count, attempts as u64);
    assert_eq!(current_runtime.completion_trace.len(), MAX_COMPLETION_TRACE);
    assert_eq!(cell.commitment().unwrap(), before);
    assert!(current_runtime.pending.is_empty());
    assert!(current_runtime.installed.is_empty());
}
...
#[test]
fn runtime_epoch_profile_is_explicit_but_uniqueness_is_external() {
    let valid = runtime_epoch("profile-valid");
    ObservationRuntime::new(valid).unwrap();

    let wrong_schema = TypedDigest32::sha256(RUNTIME_EPOCH_DOMAIN, 1, b"wrong-schema").unwrap();
    assert_eq!(
        ObservationRuntime::new(wrong_schema),
        Err(RefError::InvalidRuntimeEpochProfile)
    );

    let wrong_domain = TypedDigest32::sha256(
        "symtropy.scale-cell-001f.not-runtime-epoch.v3",
        SCALE_CELL_001F_SCHEMA_VERSION,
        b"wrong-domain",
    )
    .unwrap();
    assert_eq!(
        ObservationRuntime::new(wrong_domain),
        Err(RefError::InvalidRuntimeEpochProfile)
    );
}
