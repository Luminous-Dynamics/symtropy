    ...
    fn net_change_for_checked(&self, port: EnergyPort) -> Result<f64, EnergyAggregateError> {
        ...
        for entry in self.entries() {
            let delta = match (entry.source == port, entry.destination == port) {
                ...
                // Self-transfers are rejected by the journal, but retaining the
                // exhaustive case keeps the aggregate robust to future schemas.
...
mod tests {
    ...
    fn wholly_external_transfers_do_not_affect_boundary_net() {
        ...
        // Throughput is unrepresentable, but no energy crosses the modeled
        // boundary, so that specific aggregate remains exactly representable.
    ...
    fn deterministic_intermediate_overflow_fails_even_if_later_entry_would_cancel() {
        ...
        // Strict evidence rejects a reduction that becomes unrepresentable at
        // any deterministic step rather than relying on a later cancellation.
        assert_eq!(
            ledger.net_external_joules_checked(),
            Err(EnergyAggregateError::NonFiniteAggregate)
        );
