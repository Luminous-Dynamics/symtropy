...
pub trait MeshColliderMetadata<const D: usize> {
    fn generate_mesh_contacts(
        ...
        self_transform: &Transform<D>,
        other_shape: &dyn Shape<D>,
        other_handle: BodyHandle,
        other_transform: &Transform<D>,
...
/// Generate a multi-point contact manifold for translated shapes.
///
/// Compatibility wrapper for the original position-only API. New code should
/// use [`generate_contact_manifold_transformed`].
...
pub fn generate_contact_manifold<const D: usize>(
    ...
    generate_contact_manifold_transformed(
        shape_a,
        &transform_a,
        shape_b,
        &transform_b,
...
/// Generate a contact manifold after complete rigid transforms.
#[allow(clippy::too_many_arguments)]
pub fn generate_contact_manifold_transformed<const D: usize>(
    shape_a: &dyn Shape<D>,
    ...
    generate_contact_manifold_support_maps(&map_a, &map_b, normal, depth, body_a, body_b)
}

/// Generate a multi-point manifold from arbitrary world-space support maps.
#[allow(clippy::too_many_arguments)]
pub fn generate_contact_manifold_support_maps<const D: usize>(
    ...
    // Use the midpoint between opposing witness supports as the primary
    // contact. This remains centered in the overlap and avoids biasing torque
    // toward body A when the two surfaces are curved or oblique.
    let witness_a = shape_a.support_world(&normal);
    let witness_b = shape_b.support_world(&(-normal));
    let p0 = (witness_a + witness_b) * 0.5;
    ...
    if depth > 1e-10 {
        ...
        for dir in &perturbs {
            let w_a = shape_a.support_world(dir);
            let w_b = shape_b.support_world(&(-*dir));
            ...
            let candidate = (w_a + w_b) * 0.5;
            if candidates
                .iter()
                .any(|(q, _)| (candidate - q).norm() < DEDUP_DIST)
            {
                continue;
            }

            if candidates.len() < MAX_CONTACTS {
                candidates.push((candidate, sep));
            }
        }
    }
