...
fn epa_3d<const D: usize>(
    ...
    if simplex.len() < 4 || D < 3 {
        // TODO(epa-box-box): This bounding-sphere fallback triggers whenever GJK
        // terminates with fewer than 4 simplex points (e.g. parallel face-on-face
        // or edge-on-edge box alignments). For HyperBox-vs-HyperBox contacts it
        // overestimates penetration depth (~1.4 units for half-extent-0.5 boxes
        // that are only fractionally overlapping), causing explosive impulses that
        // prevent stable box stacking. Two viable fixes:
        //   (a) Reuse GJK's `do_simplex` Voronoi reduction to complete the simplex
        //       into a valid polyhedron as a starting point for the EPA expansion.
        //   (b) Add a SAT analytical fast path for HyperBox-vs-HyperBox in
        //       `world.rs` (analogous to the existing `contact_against_halfspace`
