...
use bevy::prelude::*;
use bevy::render::{
    render_graph::{Node, NodeRunError, RenderGraphContext, RenderGraphExt, RenderLabel, RenderSubGraph, RenderGraph},
    render_resource::*,
...
pub use fluid::{HybridFluidPlugin, HybridFluidManager, GpuFluidVoxel};
...
impl Plugin for GpuPhysicsPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(MaterialPlugin::<InstancedPhysicsMaterial>::default())
            ...
            .add_systems(Update, (upload_physics_to_gpu_sparse, cognitive_to_gpu_bridge, readback_nociception));
...
impl FromWorld for BroadphasePipeline {
    fn from_world(world: &mut World) -> Self {
        ...
        let entries = vec![
            ...
            BindGroupLayoutEntry { binding: 9, visibility: ShaderStages::COMPUTE, ty: BindingType::Buffer { ty: BufferBindingType::Storage { read_only: false }, has_dynamic_offset: false, min_binding_size: None }, count: None },
            BindGroupLayoutEntry { binding: 10, visibility: ShaderStages::COMPUTE, ty: BindingType::Buffer { ty: BufferBindingType::Storage { read_only: false }, has_dynamic_offset: false, min_binding_size: None }, count: None },
            BindGroupLayoutEntry { binding: 11, visibility: ShaderStages::COMPUTE, ty: BindingType::Buffer { ty: BufferBindingType::Storage { read_only: true }, has_dynamic_offset: false, min_binding_size: None }, count: None },
        ];

        let bind_group_layout = render_device.create_bind_group_layout(Some("gpu_physics_layout"), &entries);
        let layout_desc = BindGroupLayoutDescriptor { label: Cow::from("Layout"), entries };
        ...
        let social_decay_pipeline = pipeline_cache.queue_compute_pipeline(ComputePipelineDescriptor {
...
fn prepare_buffers(
    ...
    let (manager, fluid_manager, render_device, pipeline) = match (manager, fluid_manager, render_device, pipeline) {
        (Some(m), Some(f), Some(d), Some(p)) => (m, f, d, p),
        _ => return,
    };
    if manager.colliders.is_empty() { return; }
    ...
    let dummy_storage = render_device.create_buffer(&BufferDescriptor { label: None, size: 1024, usage: BufferUsages::STORAGE, mapped_at_creation: false });
    ...
    let bind_group = render_device.create_bind_group(
        ...
        &[
            BindGroupEntry { binding: 0, resource: collider_buffer.as_entire_binding() },
            BindGroupEntry { binding: 1, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 2, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 3, resource: config_buffer.as_entire_binding() },
            BindGroupEntry { binding: 4, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 5, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 6, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 7, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 8, resource: dummy_storage.as_entire_binding() },
            BindGroupEntry { binding: 9, resource: social_buffer.as_entire_binding() },
            BindGroupEntry { binding: 10, resource: nociception_buffer.as_entire_binding() },
            BindGroupEntry { binding: 11, resource: fluid_buffer.as_entire_binding() },
        ],
    );
...
impl Node for BroadphaseComputeNode {
    fn run(
        ...
        command_encoder.copy_buffer_to_buffer(&buffers.nociception_buffer, 0, &buffers.nociception_staging, 0, 256);
        Ok(())
    }
}

fn readback_nociception(
    mut _results: ResMut<NociceptionResults>,
) {
}
...
pub fn cognitive_to_gpu_bridge(
    ...
    for (body, transform, brain) in query.iter() {
        ...
        if idx < manager.colliders.len() as u32 && idx < manager.social_states.len() as u32 {
            ...
            manager.social_states[idx as usize].stewardship_care = brain.profile.stewardship_care as f32;
        }
    }
}
...
fn upload_physics_to_gpu_sparse(
    ...
    if time.elapsed_secs() % 5.0 < 0.02 { info!("TRACE: upload_physics_to_gpu_sparse ticking..."); }
    ...
    if manager.social_states.len() < num_bodies {
        manager.social_states.resize(num_bodies, GpuCitizenSocialState {
    ...
    if storage_buffers.get(&manager.instance_buffer).is_none() {
        ...
        manager.instance_buffer = storage_buffers.add(ShaderStorageBuffer::new(bytemuck::cast_slice(&buffer), bevy::asset::RenderAssetUsages::default()));
    }

    if storage_buffers.get(&manager.social_buffer).is_none() {
        manager.social_buffer = storage_buffers.add(ShaderStorageBuffer::new(bytemuck::cast_slice(&manager.social_states), bevy::asset::RenderAssetUsages::default()));
