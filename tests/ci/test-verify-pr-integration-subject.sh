...
(
  cd "$repo"
  bash "$VERIFIER" "$base" "$head" >/dev/null
)

echo "ok: exact two-parent integration subject accepted"

set +e
(
  cd "$repo"
  bash "$VERIFIER" "$head" "$base" >/dev/null 2>&1
)
rc=$?
set -e
[[ $rc -eq 2 ]] || { echo "expected wrong tuple to exit 2, got $rc" >&2; exit 1; }
...
(
  cd "$repo"
  bash "$VERIFIER" "$base" "$head" >/dev/null 2>&1
)
rc=$?
set -e
[[ $rc -eq 2 ]] || { echo "expected single-parent checkout to exit 2, got $rc" >&2; exit 1; }
echo "ok: non-merge checkout denied"

set +e
(
  cd "$repo"
  bash "$VERIFIER" deadbeef "$head" >/dev/null 2>&1
)
rc=$?
set -e
[[ $rc -eq 64 ]] || { echo "expected malformed SHA input to exit 64, got $rc" >&2; exit 1; }
