...
status: superseded
supersedes:
- SYSTEM_INTERACTION_AND_DEPENDENCY_MAP_V1_2.md
related:
- SYMTROPY_GAME_CONSTITUTION_V0_6.md
- HISTORICAL_CONTENT_AND_PLAYABLE_CAMPAIGN_CONTRACT_V0_1.md
...
superseded_by: SYSTEM_INTERACTION_AND_DEPENDENCY_MAP_V1_4.md
---
...
# v1.6 Interstellar Threshold Interaction Layer

## Relativistic Chronology and Causal Distance

Consumes:

```text
physical positions and trajectories
reference frames and clocks
propulsion and habitat state
message routes and relay custody
worldline ancestry
```
...
traveler chronology and reconciliation
```

No consumer may read remote current state or treat historical credentials as current authority automatically.

## Autonomous Missions and Ark Continuity

Consumes:

```text
mission charters
probes, arks, people, machines, and ecology
replication and resource envelopes
contact evidence
skill, culture, institution, and archive state
```

Produces:

```text
mission reviews and interpretations
probe and replicator actions
ark metabolism and demographic continuity
cultural and language divergence
descendant constitutional decisions
```

## Stellar Contact and Signal Evidence

Consumes:

```text
raw observations and sensor state
relay routes
processing and translation hypotheses
representation claims
contact and noninterference policy
```
...
```text
contact states
signal provenance graphs
long vows and noncontact boundaries
treaty succession and quarantine cases
public narratives with evidence boundaries
```

## Interstellar Rescue and Historical Loss

Consumes:

```text
distress evidence
trajectory and survival estimates
responder and refuge capacity
identity, body, archive, and wreck state
```
...
direct or partial aid
refuge preparation
historical obligations
memorial, recovery, and salvage boundaries
```

## Infrastructure-Locked Transit

Consumes:

```text
gate physical and power state
endpoint synchronization
physical manifests and identity roots
destination capacity
consent, authority, quarantine, and worldline binding
```

Produces:

```text
staged transit reservations
one authoritative commit outcome
arrival reconciliation
audit and Chronicle evidence
gate-access political consequences
```

## Interstellar Shared Invariants

```text
no shared immediate truth across stars
no early message delivery
no permanent remote survival authority
no descendant or machine participant treated as mission property
no replication without lineage, ceiling, and shutdown policy
no ancient message, treaty, or artifact treated as current authority automatically
no rescue ranking by wealth
no casual ship-mounted FTL
no gate or worldline operation duplicating unique people, vessels, source chains, cargo, or claims
```


# v1.7 Historical Texture Interaction Layer

## Historical Texture Graph

Consumes:

```text
physical and ecological pressures
procedural history events
institution and corporate state
population movement
archives and evidence
language, culture, art, and belief transmission
worldline ancestry
```
...
```text
named events, places, institutions, and people
physical and institutional historical scars
successor-state lineages
diaspora and shadow-network continuity
artistic and ritual responses
competing historical interpretations
player-discoverable traces
```

## Shared Invariants

```text
names do not create causes
historical knowledge follows evidence and transmission
corporate collapse does not erase physical dependency
successors do not duplicate unique assets or claims
diasporas retain internal disagreement and institutions
art and ritual require carriers and material practice
historical figures remain bounded by capability and networks
correction preserves prior evidence and narrative history
worldline variation preserves ancestry
private memory and archive content remain protected
```

## Primary Consumers

```text
procedural history and faction evolution
NPC belief, memory, and dialogue
mission and encounter generation
settlement architecture and revisit state
corporate economy and labor systems
archives, media, justice, and reputation
cultural evolution and acoustic civilization
worldline persistence and Chronicle presentation
```


# v1.8 Playable History Interaction Layer

## Campaign Packet Compiler
...
institutions, factions, and named inhabitants
services, routes, sites, and resources
archive evidence and access scopes
culture and ordinary-life practices
production, performance, localization, and accessibility budgets
worldline ancestry
```
...
```text
validated campaign packet
multidimensional pressure state
faction initiative envelopes
activities bound to real sites and capabilities
evidence and interpretation objects
ordinary-life content weave
resolution envelope
revisit presentation matrix
stable package identity and migration data
```

## Shared Invariants

```text
campaigns begin before the player arrives
narrative text does not mutate authority
factions act only through knowledge and capability
services survive leadership defeat until physically replaced
historical evidence preserves provenance and privacy
ordinary life continues during pressure
absence advances bounded causal state
outcomes preserve tradeoffs and unfinished work
worldline variants preserve ancestry and uniqueness
compilation failure falls back without inventing truth
```

## Primary Consumers

```text
mission and activity runtime
NPC cognition, memory, dialogue, and relationships
settlement and infrastructure simulation
archives, rumor, justice, and reputation
cultural seasons and acoustic civilization
revisit presentation and Chronicle
content tools, modding, QA, and playtest evidence
```


# v1.9 Lore Register and Human-Mess Interaction Layer

## Lore Register Compiler

Consumes:

```text
historical texture graph
playable campaign packet
local languages and social groups
physical and occupational context
institutions and household state
popular culture and media channels
archives, rumors, and regional memory
nonhuman agency profiles
privacy and disclosure scopes
worldline ancestry
```
...
```text
legal, local, historical, and slang names
diegetic artifacts bound to authors and purposes
ordinary institutions unrelated to the flagship civic thesis
private and irrational motive proposals
popular-culture carriers and audience networks
historical uncertainty and false-lesson records
figure private-life boundaries
nonhuman ordinary activity
blind-review and style-diagnostic evidence
```

## Authority boundaries

```text
alternate names do not change stable entity identity
irrational motives propose behavior but do not invent capability
artifacts do not become authoritative merely because collectible
popular media influences belief through transmission, not global truth
historical noise preserves causality and provenance
private-life details remain access-scoped
nonhuman humor or play labels preserve translation uncertainty
style diagnostics cannot override accessibility or rights requirements
```

## Primary consumers

```text
NPC dialogue, memory, relationships, and schedules
campaign activity generation
regional architecture and signage
media, rumor, reputation, and public history
localization and voice direction
ambient audio, music, sport, and festivals
procedural naming and content tools
QA, narrative review, cultural review, and playtest research
```

## Failure handling

If a lore-register package fails validation:

- stable world identity remains unchanged;
- the last validated names and artifacts remain available;
- generated artifacts fall back to authored local content;
- private material fails closed;
- no campaign or simulation authority is invented;
- diagnostic failure blocks content promotion, not world loading.

# v2.0 IRIS and Mature-Reality Integration

## IRIS Boundary

```text
causal observations + scoped records + arrived messages + memory
→ IRIS bounded cognition
→ advice and semantic frame
→ player or authorized policy
→ authoritative action validation
→ physical and institutional outcome
→ evidence and memory
```

IRIS consumes immutable snapshots and publishes proposals. It does not mutate world state, consent, guilt, diagnosis, custody, or source-chain identity.

## Bodily Care Exchange

```text
body health + environment + assistive devices + care plan
→ functional need
→ household or institutional labor
→ resource use and privacy-scoped outcome
→ health, fatigue, relationship, and service-state consequences
```

Care systems consume water, space, equipment, medicine, time, and trained labor. They publish only the minimum information required by each consumer.

## Adult Intimacy Exchange

```text
adult status + relationship state + current consent + capacity + power constraints
→ permitted interaction envelope
→ optional presentation
→ relationship and privacy consequences
```

Language rendering never creates consent.

## Addiction and Medication Exchange

```text
exposure + body state + treatment plan + supply access + context
→ effect, impairment, dependence, withdrawal, or recovery signals
→ grounded NPC and player decisions
→ health, work, care, legal, and relationship consequences
```

## Coercive-Control Exchange

Safety planning consumes transport, shelter, documents, money, devices, medicine, care, and trusted relationships. Adjudication consumes evidence and authority. The two paths may proceed at different thresholds.

## War and Postwar Exchange

Combat publishes damage, custody, displacement, missing-person, route, evidence, and institutional events. Civilian, justice, health, archive, migration, economy, and history systems consume them. A ceasefire does not reset those outputs.

## Mental Health and Faith Exchange

Mental-health systems publish functional needs, distress, treatment choices, capacity support, and privacy-scoped risk signals—not universal diagnoses. Belief systems publish practices, institutions, interpretations, and care without a hidden metaphysical truth score.

## Presentation Boundary

Mature presentation is downstream from authoritative state. Full, reduced, summary, and skip modes must converge on the same authoritative outcome.

## v2.0 Hard Boundary

No integration may allow:

- IRIS omniscience;
- generated consent or guilt;
- private-health leakage;
- mature presentation to alter outcome quality;
- torture or abuse as efficient information mechanics;
- care recipients becoming resources;
- worldline forks duplicating authority or identity.
