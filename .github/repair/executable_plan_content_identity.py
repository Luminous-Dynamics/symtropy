...
cargo = root / "crates/domains/symtropy-fabrication/Cargo.toml"
cargo_text = cargo.read_text()
needle = 'serde = { version = "1.0", features = ["derive"] }\nsymtropy-game-state = { path = "../symtropy-game-state" }\n'
replacement = 'serde = { version = "1.0", features = ["derive"] }\nsha2 = "0.10"\nsymtropy-game-state = { path = "../symtropy-game-state" }\n'
if needle not in cargo_text:
    raise SystemExit("fabrication Cargo.toml dependency anchor changed")
cargo.write_text(cargo_text.replace(needle, replacement, 1))

lib = root / "crates/domains/symtropy-fabrication/src/lib.rs"
lib_text = lib.read_text()
if "mod executable_plan_identity;" in lib_text:
    raise SystemExit("executable_plan_identity module already present")
lib_text = lib_text.replace("mod executable_plan;\n", "mod executable_plan;\nmod executable_plan_identity;\n", 1)
lib_text = lib_text.replace("pub use executable_plan::*;\n", "pub use executable_plan::*;\npub use executable_plan_identity::*;\n", 1)
lib.write_text(lib_text)

module = root / "crates/domains/symtropy-fabrication/src/executable_plan_identity.rs"
if module.exists():
    raise SystemExit("executable_plan_identity.rs already exists")
module.write_text(r'''// Copyright (C) 2026 Tristan Stoltz / Luminous Dynamics
...
use crate::{
    ExecutableFabricationPlan, FabricationPlanId, ProcessKind, WorkpieceLifecycle,
};
...
mod tests {
    use super::*;
    use crate::{
        CapabilityAdmissionId, CapabilityAxisNeed, CapabilityAxisRange, CapabilityEvidenceRef,
        CapabilityEnvelope, CapabilityNeed, CapabilityNeedId, CapabilityRequirement,
        ExactPlanProcessBinding, FabricationPlan, PlanDependency, PlanStep, PlanStepId, ProcessSpec,
        ProcessSpecId, WorkpieceId,
    };
    ...
    fn provider_state_is_outside_plan_digest() {
        ...
        .unwrap();
        let admission_a = need.evaluate(
            CapabilityAdmissionId::new(id("admission:a")),
            &provider_a,
        );
        let admission_b = need.evaluate(
            CapabilityAdmissionId::new(id("admission:b")),
            &provider_b,
        );
    ...
    fn round_trip_preserves_digest_and_exact_ref() {
        ...
        let restored_ref: ExecutableFabricationPlanRef = serde_json::from_slice(&encoded_ref).unwrap();
